...
<%
  ...
  ///Validar si es tercero para bloquear el registro de cita
      	...
      	pg070102021CFG validador = new pg070102021CFG();
      	bloqueaGeneraCita = validador.getBloqueaGeneraCita(vUsuario.getCUsuario(),vUsuario.getICveusuario());
...
<SCRIPT LANGUAGE="JavaScript"
	SRC="<%=vParametros.getPropEspecifica("RutaFuncs")%>valida_nt.js"></SCRIPT>

<script language="JavaScript">
 function fAlerta(){
   alert("Este Expediente se encuentra Inhabilitado para Generar Examenes en Medicina Preventiva!");
  }
 function fAlertaTerceros(expediente){
	   alert("Fueron superados los 90 días de la generación del expediente "+expediente+" para poder realizar su primer examen médico con un Tercero Particular, favor de acudir a una Unidad Médica para realizar su examen!");
	}
 function fAlertaMulta(multa,rfcing){
	 if(multa>0){
	  	alert("Es merecedor a una multa de $"+multa +", ser\u00E1 redireccionado para generar su hoja de pago. \n\n"+
	  			"Una vez realizado el pago, favor de presentar su boucher para registrar su pago.");
	  	//window.open("https://www.w3schools.com");
	  	//window.open("http://10.33.143.54:7001/e5Cinco/generaHojaAyudaInsertableVisible.jsp?hdCampoClave2="+rfcing+"&iCveDependencia=12&iCveCtrosAdms=2&iCveCUO=40&iCveAreaRecauda=8&iCveOficina=1&iCveCategoria=4&iCveTramite=8052460&iImporte="+multa+"");
	  	window.open("<%=vParametros.getPropEspecifica("URLINGHojaAyudaInsertable")%>?hdCampoClave2="+rfcing+"&iCveDependencia=12&iCveCtrosAdms=2&iCveCUO=40&iCveAreaRecauda=8&iCveOficina=1&iCveCategoria=4&iCveTramite=8052460&iImporte="+multa+"");
	  	//alert(rfcing);
	 }
	}
</script>
...
<body bgcolor="" topmargin="0" leftmargin="0" onLoad="fOnLoad();">
	<form method="POST" action="_self">
		&nbsp; <input type="hidden" name="hdBuscar" value="0"> <input
			type="hidden" name="hdCondicion" value=""> <input
			type="hidden" name="hdiCvePersonal"> <input type="hidden"
			name="hdTipo"
			value="<%if((""+request.getParameter("hdTipo")).compareTo("null") != 0) out.print(""+request.getParameter("hdTipo")); %>">
		<input type="hidden" name="hdPreDet" value="">
		<table class="ETablaInfo" background="" border="1" align="center"
			cellspacing="0" cellpadding="3">
			<% String cTipo = ""+request.getParameter("hdTipo");
       if(cTipo.compareTo("1") == 0 || cTipo.compareTo("2") == 0){
    %>
			<tr>
				<td class="ETablaT" colspan="2">Seleccione Unidad M&eacute;dica
					y M&oacute;dulo</td>
			</tr>
			<tr>
      ...
      if(cBuscar.compareTo("1") == 0){
       ...
       %>
           ...
           window.resizeTo(700,500);
			...
			<table class="ETablaInfo" border="1" align="center" cellspacing="0"
				cellpadding="3">
				<tr>
            ...
            for(int i=0;i<vcPersonal.size();i++){
              ...
              int iNumExamen = 0;
              int iNumExamenesEPI = 0;
              int iNumeroDiasPrimerTomaBiometricos = 0;
              boolean impedirImportarExpediente = false;
              try{
            	  iNumExamen = dAplica.FindByMaxExamen(vPerDatos.getICveExpediente());
            	  if(iNumExamen==0){////EL NUMERO MAXIMO DE EXAMEN ES IGUAL A 0
            		  if(bloqueaGeneraCita){ ///EL USUARIO EN SESSION ES UN MEDICO TERCERO (EXT)
           		  		iNumeroDiasPrimerTomaBiometricos = dLICFFH.NumDiasPrimerTomaBiometricos(vPerDatos.getICveExpediente());
           			  	if(iNumeroDiasPrimerTomaBiometricos > 90){///HAN TRANSCURRIDO 90 DIAS DE LA TOMA DE BIOMETRICOS DE ESTE EXPEDIENTE
           					  impedirImportarExpediente = true;
        			  	   	  //System.out.println("-impedirImportarExpediente = "+impedirImportarExpediente);
           			  	}  
           		  	  }            			  
            	  }else{
            		  if(bloqueaGeneraCita){ ///EL USUARIO EN SESSION ES UN MEDICO TERCERO (EXT)
            			  iNumExamenesEPI = dAplica.FindByCountExamenEPI(vPerDatos.getICveExpediente());
            		  	  if(iNumExamenesEPI == 0){///TIENE PRACTICADO POR LO MENOS UN EPI
            		  		iNumeroDiasPrimerTomaBiometricos = dLICFFH.NumDiasPrimerTomaBiometricos(vPerDatos.getICveExpediente());
              			  	if(iNumeroDiasPrimerTomaBiometricos > 90){///HAN TRANSCURRIDO 90 DIAS DE LA TOMA DE BIOMETRICOS DE ESTE EXPEDIENTE
              					  impedirImportarExpediente = true;
              			  	   		//System.out.println("+impedirImportarExpediente = "+impedirImportarExpediente);
            	  ...
            	  System.out.println("Error en la consulta FindByMaxExamen");
              }
              out.print("<tr>");
              ...
              out.print(vEti.Texto("ETabla",""+iNumExamen));
               ...
               //int Multa = CalculaMulta.ObteniendoMontodeLaMulta(vPerDatos.getICveExpediente());
               int Multa = 0;
				              ...
				              */
								...
								try {
									...
									if (Inhabilitado == 1) {
										...
										/*
										if (Activo == 1) {
											out.print("<td>" + vEti.clsAnclaTexto("ETabla", "" + vPerDatos.getICvePersonal(),
													"javascript:fSelected('" + vPerDatos.getICvePersonal() + "',"
															+ vPerDatos.getICveExpediente() + "," + vPerDatos.getINumExamen()
															+ ",'" + vPerDatos.getCNombreCompleto() + "');fAlerta();",
													"Selected") + "</td>");
										} else {
											if (impedirImportarExpediente) {
												out.print("<td>"
														+ vEti.clsAnclaTexto("ETabla",
																"" + vPerDatos.getICvePersonal(), "javascript:fAlertaTerceros('"
																		+ vPerDatos.getICvePersonal() + "');",
																"Selected")
														+ "</td>");
											} else {
												out.print(
														"<td>" + vEti.clsAnclaTexto("ETabla", "" + vPerDatos.getICvePersonal(),
																"javascript:fSelected('" + vPerDatos.getICvePersonal() + "',"
																		+ vPerDatos.getICveExpediente() + ","
																		+ vPerDatos.getINumExamen() + ",'"
																		+ vPerDatos.getCNombreCompleto() + "');",
																"Selected") + "</td>");
											}
										}*/
									} /*else {
								...
								if(impedirImportarExpediente){
									Restricciones = Restricciones+"javascript:fAlertaTerceros('"+ vPerDatos.getICvePersonal() + "');";
								}
								if(Multa>0){
									Restricciones = Restricciones+"javascript:fAlertaMulta('"+ Multa + "','"+ vPerDatos.getCRFC() + "');";
									System.out.println( vPerDatos.getCRFC() );
								}
								if(Activo == 1){
									Restricciones = "fAlerta();";
								}
								
								
								if(Restricciones.length()>10){
									out.print("<td>"
											+ vEti.clsAnclaTexto("ETabla",
													"" + vPerDatos.getICvePersonal(), Restricciones+"",
													"Selected")
											+ "</td>");
								}else{
									out.print(
											"<td>" + vEti.clsAnclaTexto("ETabla", "" + vPerDatos.getICvePersonal(),
													"javascript:fSelected('" + vPerDatos.getICvePersonal() + "',"
															+ vPerDatos.getICveExpediente() + ","
															+ vPerDatos.getINumExamen() + ",'"
															+ vPerDatos.getCNombreCompleto() + "');"+Restricciones+"",
													"Selected") + "</td>");
								}
								...
								out.print("<td>"
										+ vEti.clsAnclaTexto("ETabla", "Detalle",
												"javascript:fDetalle('" + vPerDatos.getICvePersonal() + "');", "Detalle")
										+ "</td>");
								out.print("</tr>");
							}
				%>
       ...
       %>
           ...
           window.resizeTo(550,390);
			...
			<center>
				<font color="Gray"> <b>No existe Personal bajo el
						criterio de b&uacute;squeda seleccionado!</b>
				</font> <br>

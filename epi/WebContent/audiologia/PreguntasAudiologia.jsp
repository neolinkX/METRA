...
<%
	...
	session.setAttribute("servicio", "68");
...
<div
	...
	<form
		...
		<div class="m-portlet__body">
				...
				<%
					contadortipodiv = 0;
								...
								if (vEXPResultados.getLObligatorio() == 1 && (vEXPResultados.getICveTpoResp()!=1 && vEXPResultados.getICveTpoResp()!=4)) {
									...
									if(vEXPResultados.getICveTpoResp()==2){
										CodigoValida2 = CodigoValida2 + CampoFinal + ":{ required: true}";
									}else{
			...
			<td colspan='6' align='center'><a class="EAncla" name=""
				href="JavaScript:fAudio('Derecho');">Oido Derecho</a></td>
			<td colspan='6' align='center'><a class="EAncla" name=""
				href="JavaScript:fAudio2('Izquierdo');">Oido Izquierdo</a></td>

			<td colspan='6' align='center'><a class="EAncla" name=""
				href="JavaScript:fAsigna(1);">Muestra Derecho </a><a class="EAncla"
				name="" href="JavaScript:fAsigna(2);"> Muestra Izquierdo</a></td>
			</tr>
		...
		<input type="hidden" name="hdOIM" value="null"> <input type="hidden"
			name="hdOIC" value="null"> <input type="hidden" name="hdOIT"
			value="null"> <input type="hidden" name="hdOIN" value="null"> <input
			type="hidden" name="hdODM" value="null"> <input type="hidden"
			name="hdODC" value="null"> <input type="hidden" name="hdODT"
			value="null"> <input type="hidden" name="hdODN" value="null"> <input
			type="hidden" name="hdGOido" value="null">
...
function guarda_signos_vitales2(){
	//alert("Listooooooooooooooooo");
	var options = {type:'question',title:'¿Desea guardar la grafica?'}
  modalConfirm(function (){
    $.ajax({
    	url: 'grdservicios/guardaServicios2.jsp',
        type: 'POST',
        data: $("#frm_svs").serialize(),
        dataType: 'json',
        success: function(resp_success){
      	if(resp_success[0].resp=='success'){
          	swal('Se guardó correctamente la grafica!', '', "success");
          	get_signosVitales();
        	}else{  swal('  '+resp_success[0].resp, '', "error");
        	}},      
        error: function(respuesta){ alerta('Alerta!','Error de conectividad de red PER-06');}
...
function guarda_signos_vitales3(){
	alert("Listooooooooooooooooo");
	var options = {type:'question',title:'¿Desea vizualizar la grafica?'}
  modalConfirm(function (){
    $.ajax({
    	url: 'grdservicios/graficasAudio.jsp',
        type: 'POST',
        data: $("#frm_svs").serialize(),
        dataType: 'json',
        success: function(resp_success){
...
<%=CodigoValida2%>
 ...
 var cStyle = '<link rel="stylesheet" href="<%= vParametros2.getPropEspecifica("RutaCSS") + vParametros.getPropEspecifica("HojaCSS") %>" TYPE="text/css">';

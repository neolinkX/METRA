...
<%
	...
	String CodigoValida = "";
	session.setAttribute("servicio", "67");
		...
		<div class="m-portlet__head-caption">
			<div class="m-portlet__head-title">
				<span class="m-portlet__head-icon"> <img
					src="img/servicios/oftalmologia.svg"
					width="25" class="svg-inject signal-strong">
				</span>
				<h3 class="m-portlet__head-text m--font-primary"> <%=cnombrerama %></h3>
			</div>
		</div>
	...
	%>
						...
						<div class="col-lg-3">
			             ...
			             <select class="form-control m-bootstrap-select" title="Seleccione" id="<%=CampoFinal%>" name="<%=CampoFinal%>" >
			                <option value="-1">Seleccione</option><%=respuestas.FindByAllOption(" icveservicio = "+session.getAttribute("servicio")+" and icverama = "+request.getParameter("hdrama") +" and icvesintoma = "+vEXPResultados.getICveSintoma()) %>
			             </select>
			          </div>
	...
	<%
					...
					<div class="form-group col-md-3">
						<label for="svs_presion_sistolica"> <%=vEXPResultados.getCPregunta()%>
							...
							data-decimal> <span> <%=vEXPResultados.getCEtiqueta()%>
						</span>
					</div>
				...
				contadortipodiv = 0;
						...
						if (vEXPResultados.getLObligatorio() == 1 && (vEXPResultados.getICveTpoResp()!=1 && vEXPResultados.getICveTpoResp()!=4)) {
									...
									if(vEXPResultados.getICveTpoResp()==2 || vEXPResultados.getICveTpoResp()==4 || vEXPResultados.getICveTpoResp()==7 ){ 
										CodigoValida = CodigoValida + CampoFinal + ":{ required: true}";
									}else{

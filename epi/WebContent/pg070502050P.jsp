  ...
  if (request.getParameter("opc") != null){
     ...
     switch(new Integer(request.getParameter("opc")).intValue()) {
        ...
        case 2:
           ...
           vPeriodo = dCTRPeriodPla.FindByAll(" WHERE CTRPeriodPla.iAnio = " + request.getParameter("val1") + " AND CTRPeriodPla.lActivo = 1 "," ORDER BY CTRPeriodPla.iCvePeriodPla ");
           if (vPeriodo.size()>0){
              ...
              for (int i = 0; i < vPeriodo.size(); i++){
                 j = j + 1;
                 vCTRPeriodPla = (TVCTRPeriodPla)vPeriodo.get(i);
                 out.println("i=form." + ObjetoDes + ".length + 1;");
                 out.println("form." + ObjetoDes + ".length=i;");
                 out.println("form." + ObjetoDes + "[" + j  + "].text=" + "\"" + vCTRPeriodPla.getcDscPeriodPla() + "\"" +";");

;(function ($, window, document, undefined) {
	...
	$.wcpTourEventStarted = function() {
		...
		editor.redraw();
		$.wcpEditorOpenMainTabWithName('Forma');
	}
	...
	$.wcpTourEventStepWillChange = function(step) {
		if (step == 'Publish') {
			...
			$.wcpEditorOpenMainTabWithName('Mapa de imagen');
		} else {
			// Open shape settings
			$.wcpEditorOpenMainTabWithName('Forma');
		}
		...
		if (formName == 'Image Map Settings') {
			...
			if (m.image.image_url && m.image.image_url.length > 0 && settings.image.url !== m.image.image_url) {
				...
				loadImage(editor.canvasImage, function() {
					// Image is loading
					// Show loader
					$.wcpEditorPresentLoadingScreen('Cargando imagen...');
				}, function() {
					// Image has loaded
					// init canvas events
					...
					$.wcpEditorHideLoadingScreenWithMessage('¡Error al cargar imagen!', true, false);
				});
			} else {
	...
	$.wcpEditorEventSaveButtonPressed = function() {
		...
		$.imp_editor_storage_store_save(editor.getCompressedSettings(), function(success) {
			if (success) {
				$.wcpEditorHideLoadingScreenWithMessage('¡Guardado!', false, false);
			} else {
	...
	$.wcpEditorEventListItemTitleButtonPressed = function(buttonName) {
		...
		if (buttonName == 'delete') {
			...
			$.wcpEditorPresentModal({
				...
				body: '¿Desea eliminar esta forma?'
			});
		}

		if (buttonName == 'rename') {
			var html = '<div class="wcp-editor-form-control">';
			html += '<label>Nombre de la forma</label>';
	...
	$.wcpEditorEventModalButtonClicked = function(modalName, buttonName) {
		...
		if (modalName == 'confirm-rename-shape') {
			...
			if (newTitle.length == 0) {
				$('#rename-shape-error').show().html('Ingrese un nuevo nombre para la forma u oprima cancelar.');
				return;
			}
			...
			if (shapeTitleExists || newTitle.length == 0) {
				$('#rename-shape-error').show().html('El nombre de la forma ya existe');
				return;
			}
	...
	$.wcpEditorEventCreatedNewInstance = function(instanceName) {
		...
		$.wcpEditorPresentLoadingScreen('Creando...');
		...
		$.imp_editor_storage_store_save(editor.getCompressedSettings(), function() {
			$.imp_editor_storage_set_last_save(settings.id, function() {
				...
				$.wcpEditorHideLoadingScreenWithMessage('¡Creado!', false, false);
			});
		});
	}
	...
	$.wcpEditorEventLoadSaveWithID = function(saveID) {
		$.wcpEditorPresentLoadingScreen('Cargando mapa de imagen...');

		$.imp_editor_storage_get_save(saveID, function(save) {
			if (!save) {
				$.wcpEditorHideLoadingScreenWithMessage('Error al cargar mapa de imagen.', true, false);
			} else {
				settings = save;
			...
			$.imp_editor_storage_get_last_save(function(lastSaveID) {
				if (lastSaveID && parseInt(lastSaveID, 10) != 0 && !isNaN(parseInt(lastSaveID, 10))) {
					...
					$.imp_editor_storage_get_save(parseInt(lastSaveID, 10), function(save) {
						if (!save) {
							$.wcpEditorHideLoadingScreenWithMessage('Error al cargar el mapa de imagen.', true, false);
						} else {
							settings = save;
							editor.launch();
						}
	...
	Editor.prototype.redrawTempPoly = function() {
		...
		if (localStorage['image-map-pro-seen-poly-tooltip'] != 1) {
			...
			html += '<div id="imp-poly-tooltip" style="left: '+ x +'px; top: '+ y +'px;">Haga click sobre el primer punto o presione enter para finalizar <i class="fa fa-times" aria-hidden="true" id="imp-poly-tooltip-close-button"></i></div>';
	...
	Editor.prototype.updateShapesForm = function() {
		...
		if (s) {
			...
			$.wcpEditorSetContentForTabWithName('Forma', form);

			$.wcpEditorUpdateForm('Shape Settings');

			// Hack - select control doesn't have an API to change the values
			var selectOptions = '<option value="">(Sin conectar)</option>';
			...
			$.wcpEditorSetContentForTabWithName('Forma', '<div class="wcp-editor-form-tabs-content-wrap">No se ha seleccionado ningna forma</div>');
		}
	}
	...
	Editor.prototype.launchTooltipContentBuilder = function() {
		if ($('#imp-editor-tooltip-content-builder-wrap').length == 0) {
			...
			html += '       <p>Presione el boton Listo cuando acabe de editar, o haga click en el boton cerrar de la esquina superior derecha.</p>';
			html += '   </div>';
			html += '   <div class="wcp-editor-control-button" id="imp-editor-done-editing-tooltip">Listo</div>';
			html += '</div>';

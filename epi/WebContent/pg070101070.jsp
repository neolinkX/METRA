...
<html>
<%
  pg070101070CFG  clsConfig = new pg070101070CFG();                 // modificar

  TEntorno    vEntorno      = new TEntorno();
  vEntorno.setNumModulo(07);
  vEntorno.setOnLoad("fOnLoad();");
  vEntorno.setNombrePagina("pg070101070.jsp");                    // modificar
  vEntorno.setArchFuncs("FValida");
  vEntorno.setArchTooltips("07_Tooltips");
  vEntorno.setMetodoForm("POST");
  vEntorno.setActionForm("pg070101070.jsp\" target=\"FRMCuerpo"); // modificar
  ...
  String cCatalogo = "pg070101071.jsp";
...
if (bs != null){
         ...
         */%>
         ...
         <%
                      ...
                      if(request.getParameter("iCveProceso") != null){
                         vMedServExamen = dMedServExamen.FindByAll(" Where a.iCveProceso = "+request.getParameter("iCveProceso").toString());
                      }else{
                         vMedServExamen = dMedServExamen.FindByAll(" Where a.iCveProceso = "+iCveP);
                      }
                      if (vServicios.size() > 0){
                         for (int z = 0; z < vServicios.size(); z++){
                             boolean flaged = false;
         ...
         <%
                             ...
                             if (iCuenta == vMotivos.size()){
                                ...
                                %>
                                    ...
                                    formx.<%=vTVM.getCDscMotivo().replace(' ','_')%><%=z%>.checked=false;
                                    </script>
                                <%
                                }
                             }
      ...
      if (vMots.size() > 0){
         for (int j = 0; j < vSrvs.size(); j++){
             ...
             for (int k = 0; k < vMots.size(); k++){
                 vTVMots = (TVGRLMotivo)vMots.get(k);
%>
                 if(form.<%=vTVMots.getCDscMotivo().replace(' ','_')%><%=j%>.checked){

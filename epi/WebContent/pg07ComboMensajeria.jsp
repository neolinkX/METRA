...
<%  int    iOpc   = request.getParameter("iOpc") != null && request.getParameter("iOpc").length() > 0 ? Integer.parseInt(request.getParameter("iOpc").toString()) : 0;
    ...
    switch  (iOpc){
      case 2: // Módulo
              ...
              int iCveUniMed = request.getParameter("iFiltro1") != null && request.getParameter("iFiltro1").length() > 0 ? Integer.parseInt(request.getParameter("iFiltro1").toString()) : 0;
              vcCombo = vUsuario.getVModXUnidad(iCveUniMed);
              break;
      case 3: // Módulo filtrado por Unidad Médica y Proceso
              // Obtener Clave de la Unidad Médica
              //iCveUniMed = request.getParameter("iFiltro1") != null && request.getParameter("iFiltro1").length() > 0 ? Integer.parseInt(request.getParameter("iFiltro1").toString()) : 0;
              //System.out.println("Casos 3");
              int iCveTipoEnvio = request.getParameter("iFiltro1") != null && request.getParameter("iFiltro1").length() > 0 ? Integer.parseInt(request.getParameter("iFiltro1").toString()) : 0;
              int iCveProceso = request.getParameter("iFiltro3") != null && request.getParameter("iFiltro3").length() > 0 ? Integer.parseInt(request.getParameter("iFiltro3").toString()) : 0;
              //vcCombo = vUsuario.getModuloFUP(iCveUniMed,iCveProceso);
              String cWhere = "";
				if (iCveTipoEnvio > 2){
              ...
              vcCombo = new TDToxMensajeria().getMensajeriaCombo(cWhere);
              break;
      case 4: // Proceso, Filtrado por Unidad Médica y Módulo
              // Obtener Clave de la Unidad Médica
              System.out.println("Casos 4");
              iCveUniMed = request.getParameter("iFiltro1") != null && request.getParameter("iFiltro1").length() > 0 ? Integer.parseInt(request.getParameter("iFiltro1").toString()) : 0;
              int iCveModulo = request.getParameter("iFiltro2") != null && request.getParameter("iFiltro2").length() > 0 ? Integer.parseInt(request.getParameter("iFiltro2").toString()) : 0;
              vcCombo = vUsuario.getVProcXModulo(iCveUniMed,iCveModulo);
              break;
      case 5: // Servicios especificos del Laboratorio
              // Se filtra por Unidad Médica, Modulo, Proceso
              System.out.println("Casos 5");
              iCveUniMed = request.getParameter("iFiltro1") != null && request.getParameter("iFiltro1").length() > 0 ? Integer.parseInt(request.getParameter("iFiltro1").toString()) : 0;
              iCveModulo = request.getParameter("iFiltro2") != null && request.getParameter("iFiltro2").length() > 0 ? Integer.parseInt(request.getParameter("iFiltro2").toString()) : 0;
              iCveProceso = request.getParameter("iFiltro3") != null && request.getParameter("iFiltro3").length() > 0 ? Integer.parseInt(request.getParameter("iFiltro3").toString()) : 0;
              vcCombo = new TDGRLUSUMedicos().findServEsp(vUsuario.getICveusuario(),iCveUniMed,iCveProceso, iCveModulo, "EPIServLab");
              break;
    }
...
<%  if(vcCombo!=null && vcCombo.size()>0){
  ...
  <%
      for(Enumeration eCombo=vcCombo.elements();eCombo.hasMoreElements();){
        TVDinRep vDinamico=(TVDinRep)eCombo.nextElement();
%>          cCombo.options[cCombo.length]=new Option("<%=vDinamico.get("cDescripcion")%>","<%=vDinamico.get("iClave")%>");
<%    }
%>

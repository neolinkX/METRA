  ...
  if (request.getParameter("opc") != null){
     ...
     switch(new Integer(request.getParameter("opc")).intValue()) {
      ...
      case 7:
           ...
           vcPuestoN = dPuestoN.FindByAll(request.getParameter("val1").toString(),request.getParameter("val2").toString());
  ...
  // Código de Verificación al Catálogo de No Aptos
        ...
        if (! vcLicencia.isEmpty()){
           ...
           if ( vcNoApto.size() > 0){
            ...
            out.println("for(i=0" + ";i<form.iCveMotivo.length;i++){");
  ...
  case 1:
      ...
      case 3:
       ...
       case 4:
           ...
           vcModulo = dModulo.FindByAll(" where iCveUniMed = " + request.getParameter("val1").toString());
         ...
         //Proceso
           ...
           int iUsuario = Integer.parseInt(request.getParameter("val2"));
           int iUniMed = Integer.parseInt(request.getParameter("val1"));
           vcUMUsuario = dGRLUMUsuario.getProcesos(iUsuario,iUniMed);
           out.println("form = window.parent.parent.FRMCuerpo.FRMDatos.document.forms[0];");
           j = 0;
       if (vcUMUsuario.size() > 0){
           out.println("form.iCveProceso.length=1;");
           out.println("form.iCveProceso[0].text=" + "\"" + "Seleccione..." + "\"" + ";");
           out.println("form.iCveProceso[0].value=" + "\"0\"" + ";");
           for (int i = 0; i < vcUMUsuario.size(); i++){
              j = j + 1;
              vGRLUMUsuario = (TVGRLUMUsuario) vcUMUsuario.get(i);
              out.println("i=form.iCveProceso.length + 1;");
              out.println("form.iCveProceso.length=i;");
              out.println("form.iCveProceso[" + j  + "].text=" + "\"" + vGRLUMUsuario.getCDscProceso() + "\"" +";");
              out.println("form.iCveProceso[" + j + "].value="+ "\"" + vGRLUMUsuario.getICveProceso()  + "\"" +";");
           }
        }else{
           out.println("form.iCveProceso.length=1;");
           out.println("form.iCveProceso[0].text=" + "\"" + "Datos no disponibles" + "\"" + ";");
           out.println("form.iCveProceso[0].value=" + "\"0\"" + ";");
        }

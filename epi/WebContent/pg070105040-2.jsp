...
System.out.println("Constancia");
	...
	pg070105040CFG2 clsConfig = new pg070105040CFG2(); // modificar
	...
	int lDictaminado = 0;
	int lCatDic = clsConfig.lDictamenCatDos(session.getAttribute("id")+"",session.getAttribute("episodio")+"");
	lDictaminado = clsConfig.lDictaminadoEADos(session.getAttribute("id")+"",session.getAttribute("episodio")+"");
	...
	vcExpDicExam = clsConfig.listaExaCatDos(session.getAttribute("id")+"",session.getAttribute("episodio")+"");
	...
	try {
		...
		WhereD = " Where iCveExpediente = "
				...
				+ " and iNumExamen = "
				+ (session.getAttribute("episodio")+"").toString() + " ";
	...
	vcMEDSERVObligatorio = dMEDSERVObligatorio.FindByAll((session.getAttribute("id")+"").toString(), (session.getAttribute("episodio")+"").toString());

	////////////Diagnosticos Diabetes //////////////////
	TDEXPDiagnostico dEXPDiagnostico = new TDEXPDiagnostico();
	boolean DiagnosticoDiabetes =  false;
	DiagnosticoDiabetes = dEXPDiagnostico.Diabetes((session.getAttribute("id")+"").toString(), (session.getAttribute("episodio")+"").toString());
	...
	int NumExpe = Integer.parseInt((session.getAttribute("id")+"").toString().trim());
	int NumExa = Integer.parseInt((session.getAttribute("episodio")+"").toString().trim());
	...
	try {
		...
		if (procesomed == 2) {
			...
			vcEXPServicio = dEXPServicio.getLConcluido(
			(session.getAttribute("id")+"").toString(), 
			(session.getAttribute("episodio")+"").toString(), "31");
			...
			if (Motivo == 1 || Motivo == 6 || Motivo == 5) {
				...
				vcEXPServicio = dEXPServicio.getLConcluido((session.getAttribute("id")+"").toString(), (session.getAttribute("episodio")+"").toString(), "67");
				for (int i=0;i<vcEXPServicio.size();i++){
				      vEXPServicio = (TVEXPServicio) vcEXPServicio.get(i);
				   }  
				...
				ServEVOFTA = dEXPExamAplica.RegresaInt("SELECT COUNT(LLOGICO) FROM EXPRESULTADO "+ 
													  ...
													  " ICVERAMA = 1 AND		"+				
													  " ICVESINTOMA = 24 AND	"+					
													  " ICVEEXPEDIENTE = "+(session.getAttribute("id")+"").toString()+" AND		"+				
													  " INUMEXAMEN = "+(session.getAttribute("episodio")+"").toString()+" AND				"+
													  " LLOGICO = 1 AND				"+		
													  " (SELECT LCONCLUIDO FROM EXPSERVICIO WHERE ICVESERVICIO = 59 AND ICVEEXPEDIENTE = "+(session.getAttribute("id")+"").toString()
													  +" AND INUMEXAMEN = "+(session.getAttribute("episodio")+"").toString()+") = 0"); 
				
				
				//EVALUACIÓN Y CONTROL DEL PACIENTE CON DIABETES MELLITUS (75)
				vcEXPServicio = dEXPServicio.getLConcluido((session.getAttribute("id")+"").toString(), (session.getAttribute("episodio")+"").toString(), "75");
				for (int i=0;i<vcEXPServicio.size();i++){
				      vEXPServicio = (TVEXPServicio) vcEXPServicio.get(i);
				   }  
		...
		if ((iCveModulo+"").toString().equals("14"))
			if ((iCveUniMed+"").toString().equals("1")) {
				ServGABOFTA = 1; //Gabinete de Oftalmologia (67)
...
<body bgcolor="<%=vParametros.getPropEspecifica("ColorFondoPagina")%>"
	...
	<form class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed"
		...
		<input type="hidden" name="hdICveExpediente"
			value="<%=(session.getAttribute("id")+"") != null ? 
			(session.getAttribute("id")+"") : ""%>">
		<input type="hidden" name="hdINumExamen"
			value="<%=(session.getAttribute("episodio")+"") != null ? 
			(session.getAttribute("episodio")+"") : ""%>">

		<input type="hidden" name="hdICveUniMed"
			value="<%=(iCveUniMed+"").toString() != null ? 
			(iCveUniMed+"").toString() : 
			(iCveUniMed+"").toString() != null ? 
			(iCveUniMed+"").toString() : ""%>">
		<input type="hidden" name="hdICveModulo"
			value="<%=(iCveModulo+"").toString() != null ? 
			(iCveModulo+"").toString() : 
			(iCveModulo+"").toString() != null ? 
			(iCveModulo+"").toString() : ""%>">
		...
		<input type="hidden" name="iCveProceso"
			value="1">
		<input type="hidden" name="iCvePersonal" value="<%=session.getAttribute("id")%>">
		...
		System.out.println("Constancia4-2");
			...
			if ((session.getAttribute("id")+"") != null
					&& (session.getAttribute("id")+"").length() > 0) {
			...
			if ((iCveServicio+"") != null
					&& (iCveServicio+"").length() > 0) {
				out.print(" <input type=\"hidden\" name=\"iCveServicio\" value=\""
						+ (iCveServicio+"") + "\"> \n");
				iCveServicio = Integer.parseInt((iCveServicio+"").toString());
			} else {
				out.print(" <input type=\"hidden\" name=\"iCveServicio\" value=\"1\"> \n");
				iCveServicio = 1;
			}
			System.out.println("Constancia4-22");
			if ((session.getAttribute("episodio")+"").toString() != null
					&& (session.getAttribute("episodio")+"").toString().length() > 0) {
				out.print(" <input type=\"hidden\" name=\"iNumExamen\" value=\""
						+ (session.getAttribute("episodio")+"").toString() + "\"> \n");
				iNumExamen = Integer.parseInt((session.getAttribute("episodio")+"").toString());
			} else {
			...
			if ("1" != null
				...
				out.print(" <input type=\"hidden\" name=\"iCveProceso\" value=\""
						+ "1" + "\"> \n");
				iCveServicio = Integer.parseInt("1".toString());
			} else {
				out.print(" <input type=\"hidden\" name=\"iCveProceso\" value=\"1\"> \n");
				iCveServicio = 1;
			}
			System.out.println("Constancia4-222");
			if ((iCveUniMed+"").toString() != null
					&& (iCveUniMed+"").toString().length() > 0) {
				out.print(" <input type=\"hidden\" name=\"iCveUniMed\" value=\""
						+ (iCveUniMed+"").toString() + "\"> \n");
				iCveUniMed = Integer.parseInt((iCveUniMed+"").toString());
			} else {
			...
			System.out.println("Constancia4-3");
		%>
		...
		<table
			...
			<input type="hidden" name="hdBoton" value="Guardar"></input>
			<%
				if (clsConfig.getAccesoValido()) {
					...
					vcEXPListaCat = clsConfig.listaCategoriaDos(session.getAttribute("id")+"",session.getAttribute("episodio")+"");
			...
			<tr>
				<td></td>
				<td valign="top">
					<table border="0" class="ETablaInfo" align="left" cellspacing="0"
						cellpadding="3">
						<%
							// Inicio de Datos
						%>
					...
					<table border=0" class="ETablaInfo" align="left" cellspacing="0"
								...
								if (lDictaminado == 0 && vcMEDSERVObligatorio.size() == 0) {
									...
									if (dtToday.compareTo(DtBlqDictamen) == 0) {
										...
										if (DiagRec == 1 && vcMEDSERVObligatorio.size() == 0) {/////DIAGNOSTICO Y RECOMENDACIONES APTO
											...
											vcPERDatos2 = dPERDatos2.FindByAll(session.getAttribute("id")+"");
						...
						<%
									...
									System.out.println(vcEXPDicSer.size());
									System.out.println(vcMEDSERVObligatorio.size());
 	...
 	if (clsConfig.getAccion().compareToIgnoreCase(
		...
		<button type="submit" class="btn btn-primary" onClick="fValidaTodo()">Guardar</button>
...
<% 			
 	...
 	System.out.println("Constancia7-1");
 ...
 %>
					<table border="0" class="ETablaInfo" align="center" cellspacing="0"
						cellpadding="3">
						<%
							// Inicio de Datos
						%>
 	...
 	System.out.println("Constancia7-2");
 ...
 %>
  ...
  function activa(str){
	...
	var sAux="";
	var idele = 0;
	var frm = document.getElementById("frm_svs");
...
//	alert(idele);
	...
	campo.checked="true";
	//alerta('Alerta!','Error de conectividad de red USR-03');

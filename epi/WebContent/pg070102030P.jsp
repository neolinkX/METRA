...
<script LANGUAGE="JavaScript">
  function fOnLoad(){
    try{
      ...
      if(forma){
<%  String cAxn=request.getParameter("idAxn");
    String cCombo="";
    Vector vcCombo=null;
    if("BUILD_MODS".equalsIgnoreCase(cAxn)){
      String cCveUniMed=request.getParameter("iCveUniMed");
      vcCombo=new pg070102030CFG().getModulos(cCveUniMed);
      cCombo="iCveModulo";
    }else if("BUILD_HRS".equalsIgnoreCase(cAxn)){
      String cCveUniMed=request.getParameter("iCveUniMed");
      String cCveModulo=request.getParameter("iCveModulo");
      String cFecha    =request.getParameter("dtFecha"   );
      vcCombo=new pg070102030CFG().getHorasDeCitas(cCveUniMed,cCveModulo,cFecha);
      cCombo="cHora";
    }else{
      cCombo="cNoExistente";
    }
...
<%  if(vcCombo!=null && vcCombo.size()>0){
      for(Enumeration eCombo=vcCombo.elements();eCombo.hasMoreElements();){
        TVDinamico vDinamico=(TVDinamico)eCombo.nextElement();
%>          cCombo.options[cCombo.length]=new Option("<%=vDinamico.get("cDescripcion")%>","<%=vDinamico.get("cIndice")%>");
<%    }
%>          cCombo.options[cCombo.length]=new Option("TODOS","");
<%  }else{

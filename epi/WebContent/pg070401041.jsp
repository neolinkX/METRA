<%/**
 * Title: Catálogo de Situación
 * Description: JSP para mostrar el Catálogo de Situación
 * Copyright: ?
...
<html>
<%
  pg070401041CFG clsConfig=new pg070401041CFG();                      // modificar
  TEntorno       vEntorno =new TEntorno();
  vEntorno.setNumModulo      (07);
  vEntorno.setNombrePagina   ("pg070401041.jsp");                     // modificar
  vEntorno.setArchTooltips   ("07_Tooltips");
  vEntorno.setOnLoad         ("fOnLoad();");
  vEntorno.setArchFuncs      ("FValida");
  vEntorno.setMetodoForm     ("POST");
  vEntorno.setActionForm     ("pg070401041.jsp\" target=\"FRMCuerpo");// modificar
  vEntorno.setUrlLogo        ("Acerca");
  vEntorno.setBtnPrincVisible(true);
  vEntorno.setArchFCatalogo  ("FFiltro");
  vEntorno.setArchAyuda      (vEntorno.getNombrePagina());

  //String cCatalogo   ="pg070401041.jsp";             // no aplica
  String cOperador   ="1";                             // modificar
  String cDscOrdenar ="Clave|Descripción|";            // modificar
  String cCveOrdenar ="iCveSituacion|cDscSituacion|";  // modificar
  String cDscFiltrar ="Clave|Descripción|";            // modificar
  String cCveFiltrar ="iCveSituacion|cDscSituacion|";  // modificar
  ...
  function fValidaTodo(){
    ...
    if(form.hdBoton.value != 'Cancelar'){
      ...
      if(form.cDscSituacion){
        cVMsg += fSinValor(form.cDscSituacion,2,'Descripción', true);
        fMayus(form.cDscSituacion);
      }
      if(cVMsg!=''){
...
<form method="<%= vEntorno.getMetodoForm() %>" action="<%= vEntorno.getActionForm() %>">
  ...
  <input type="hidden" name="iCveSituacion" value="<%=bs!=null?(String)bs.getFieldValue("iCveSituacion",""):""%>">
...
%>    <tr><td>&nbsp;</td></tr>
    <tr><td valign="top">
      <table border="1" class="ETablaInfo" align="center" cellspacing="0" cellpadding="3">
        <tr class="ETablaT"><td colspan="2">Situaci&oacute;n del Personal</td></tr>
<%    if(lNuevo||bs!=null){
        String iCveSituacion=lNuevo?"":(String)bs.getFieldValue("iCveSituacion","");
        String cDscSituacion=lNuevo?"":(String)bs.getFieldValue("cDscSituacion","");
        String lActivo      =lNuevo?"1":(String)bs.getFieldValue("lActivo"      ,"");
%>        <tr><%=vEti.EtiCampo("EEtiqueta","Clave:","Ecampo","text",5,5,"iCveSituacion",iCveSituacion,3,"","",false,false,lNuevo||lCaptura)%></tr>
        <tr><%=vEti.EtiCampo("EEtiqueta","Descripci&oacute;n:","Ecampo","text",20,20,"cDscSituacion",cDscSituacion,3,"","",false,true,lNuevo||lCaptura)%></tr>

...
<%
  ...
  vEntorno.setNombrePagina("pg070305075.jsp");                    // modificar Ok
  vEntorno.setArchTooltips("07_Tooltips");
  vEntorno.setOnLoad("fOnLoad();");
  vEntorno.setArchFuncs("FValida");
  vEntorno.setMetodoForm("POST");
  vEntorno.setActionForm("pg070305075.jsp\" target=\"FRMCuerpo"); // modificar Ok
...
%>    <tr><td>&nbsp;</td></tr>
    <tr><td valign="top"><table border="1" class="ETablaInfo" align="center" cellspacing="0" cellpadding="3">
      <tr><td class="ETablaT" colspan="5">Resultado Toxicologico</td></tr>
...
<%
          String condicionnueva=" where M.iCvePersonal = "+request.getParameter("hdICvePersonal")+" order by M.iAnio, M.iCveMuestra ";
          Vector vDespliega = new Vector();
          TVMuestra VMuestra;
       try{
          //vDespliega = new TDTOXMuestra().findInfoGeneral(cCondicion);
          vDespliega = new TDTOXMuestra().findInfoGeneral(condicionnueva);      
          String Resultado = "";
          int fecha=10;
          String Leyenda ="";
          ...
          if(VMuestra.getCDscSituacion().toString().equals("RECHAZADA")){
                  Resultado=" ";  
          }
          else{
               Resultado=VMuestra.getCResultado(); 
               }
               
          if(Resultado.equals("Positiva") && i == 0)
              {
                    TDEXPExamCat dEXPExamCat = new TDEXPExamCat();      
                    String RegresaFecha = " ";
                    String cWhere = "WHERE"+
                             "           EA.ICVEEXPEDIENTE = "+request.getParameter("hdICvePersonal")+"		AND"+
                             "           EA.INUMEXAMEN =	EC.INUMEXAMEN                                   AND"+
                             "           EA.ICVEEXPEDIENTE = EC.ICVEEXPEDIENTE                                  AND"+
                             "           EA.ICVEEXPEDIENTE = EC.ICVEEXPEDIENTE                                  AND"+
                             "           EA.DTSOLICITADO > '"+VMuestra.getDtRecoleccion().toString()+"'         AND"+
                             "           EA.ICVEPROCESO = 1                                                     AND"+
                             "           EC.ICVEMOTIVO = 5		";
                           
                    RegresaFecha = dEXPExamCat.FindByRevaloracion(cWhere);
                    if(RegresaFecha.length() == 10){
                        Leyenda = "Se presento a examen de revaloración el día " +RegresaFecha+ "";
                   }else{
                        Leyenda = "No se ha presentado examen de Revaloración";
                   }
 %>
                  <tr class="ETabla">
                    <td align='center'c colspan='4'><%=Leyenda%></td>
                  </tr>

 <% 
System.out.println("Captura |"+VMuestra.getDtCaptura() + "|"+VMuestra.getCDtCaptura());              
            //        System.out.println("Desecho |"+VMuestra.getDtDeshecho()+"|");              
            //        System.out.println("Recoleccion |"+VMuestra.getCDtRecoleccion()+"|"+VMuestra.getCDtRecoleccion());              
            //        System.out.println("Recoleccion2 |"+VMuestra.getdtRecoleccion()+"|");                          
...
<tr class="ETabla">
    <td><%=VMuestra.getDtRecoleccion()%></td>
    <td><%=VMuestra.getCDscUM()%></td>
    <td><%=VMuestra.getCDscModulo()%></td>
    <td><%=VMuestra.getCDscMotivo()%></td>
    <td><%=VMuestra.getCDscSituacion()%></td>
    <td><%=Resultado%></td>
    </tr>
        </tr>
<%      }
...
<%   
      ...
      TDEXPExamCat dEXPExamCat = new TDEXPExamCat();
      TVDICTAMEN vDICTAMEN = new TVDICTAMEN();
      Vector vcDICTAMEN = new Vector();
      vcDICTAMEN = dEXPExamCat.FindByDictamen("WHERE "+
                                              "  EA.ICVEEXPEDIENTE = "+ request.getParameter("hdICvePersonal")+"	AND"+
                                              "  EA.ICVEEXPEDIENTE = EC.ICVEEXPEDIENTE                                 	AND"+
                                              "  EA.ICVEEXPEDIENTE = EC.ICVEEXPEDIENTE                           	AND"+
                                              "  EA.INUMEXAMEN =	EC.INUMEXAMEN                                   AND"+
                                              "  EA.ICVEPROCESO = GRLM.ICVEPROCESO					AND"+
                                              "  EC.ICVEMOTIVO = GRLM.ICVEMOTIVO					AND"+
                                              "  EA.ICVEPROCESO = GRLP.ICVEPROCESO                                         "+
                                              "  ORDER BY EA.INUMEXAMEN DESC");
System.out.println(vcDICTAMEN.size());
         if(vcDICTAMEN.size() > 0){  
          for(int j=0;j<1;j++){
          vDICTAMEN = (TVDICTAMEN) vcDICTAMEN.get(j);
          String ResultadoDictamen="";
          if(vDICTAMEN.getLDictamen() == 0)
              ResultadoDictamen = "NO APTO";
         if(vDICTAMEN.getLDictamen() == 1)
              ResultadoDictamen = "APTO";
%>
<tr><td valign="top"><table border="1" class="ETablaInfo" align="center" cellspacing="0" cellpadding="3">
      <tr class="ETablaT">
        <td>Fecha de último Examen</td>
        <td>Motivo</td>
         <td>Resultado</td>
</tr>  
<tr class="ETabla">
    <td><%=vDICTAMEN.getDTSolicitado()%></td>
    <td><%=vDICTAMEN.getCDscMotivo()%></td>
    <td><%=ResultadoDictamen%></td>
    </tr>
        </tr>
</table></td></tr>        
<%      }

  ...
  function fBuscar(){
    ...
    if(frm.iTipo.value == 1){
      if(confirm("El reporte tomará algunos minutos en crearse ¿Desea continuar?"))
        fShowReport('pg070202040R.jsp',1,cDscUniMed + ', ' + cDscModulo + ', EXÁMENES MÉDICOS EN OPERACIÓN DEL AÑO: ' + frm.iAnio.value,'',frm.iAnio.value,1);
    }
    if(frm.iTipo.value == 2){
      if(confirm("El reporte tomará algunos minutos en crearse ¿Desea continuar?"))
        fShowReport('pg070202040R.jsp',2,cDscUniMed + ', ' + cDscModulo + ', EXÁMENES MÉDICOS EN OPERACIÓN DEL MES: ' + cMeses,'',frm.iAnio.value + '|' + frm.iMeses.value,1);
    }
    if(frm.iTipo.value == 3){
      if(fValFecha(frm.dtFecha.value)){
        fShowReport('pg070202040R.jsp',3,cDscUniMed + ', ' + cDscModulo + ', EXÁMENES MÉDICOS EN OPERACIÓN DEL DÍA: ' + frm.dtFecha.value,'',frm.dtFecha.value,1);
      }
      else

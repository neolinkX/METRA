  ...
  function fBuscar(){
    ...
    if(frm.iTipo.value == 1){
      if(confirm("El reporte tomara¡ algunos minutos en crearse ¿Desea continuar?"))
        fShowReport('pg070106020R.jsp',1,cDscUniMed + ', ' + cDscModulo + ', EXÁMEN PSICOFÍSICO INTEGRAL DEL AÑ‘O: ' + frm.iAnio.value,'',frm.iAnio.value,1);
    }
    if(frm.iTipo.value == 2){
      if(confirm("El reporte tomara¡ algunos minutos en crearse ¿Desea continuar?"))
        fShowReport('pg070106020R.jsp',2,cDscUniMed + ', ' + cDscModulo + ', DICTAMENES EMITIDOS DEL MES: ' + cMeses,'',frm.iAnio.value + '|' + frm.iMeses.value,1);
    }
    if(frm.iTipo.value == 3){
      if(fValFecha(frm.dtFecha.value)){
        fShowReport('pg070106020R.jsp',3,cDscUniMed + ', ' + cDscModulo + ', EXÁMEN PSICOFÍSICO INTEGRAL DEL DÍA: ' + frm.dtFecha.value,'',frm.dtFecha.value,1);
      }
      else
        alert('La fecha contiene un dato no valido!');
    }
  }

  function fBuscar(op){
    ...
    lAcceso = true;
    cCondicion = '';
    palabras = form.cTitulo.value;
    var cadenas = palabras.split(' ');
    var consulta = '';
    
    if(form.cTitulo.value != ''){
    	if(op == 1){
    		for (i = 0; i < cadenas.length; i++) {
    			if(i==0){
    				consulta = consulta +" CDSAYUDA like '%" + cadenas[i] + "%'" ;
    			}else{
    				consulta = consulta +" or CDSAYUDA like '%" + cadenas[i] + "%'" ;
    			}
    	    }
    			/*cCondicion = " where " +consulta;*/
        }
    	if(op == 2){
    		for (i = 0; i < cadenas.length; i++) {
    			if(i==0){
    				consulta = consulta +" CDSCDESCRIPCION like '%" + cadenas[i] + "%'" ;
    			}else{
    				consulta = consulta +" or CDSCDESCRIPCION like '%" + cadenas[i] + "%'" ;
    			}
    	    }
    			/*cCondicion = " where " +consulta;*/
    	}
    ...
    if(form.hdAdmin.value == 'true'){
    	cCondicion = " where " +consulta;
    }else{
    	cCondicion = " where (" +consulta+") AND LADMIN = 0 ";
    }

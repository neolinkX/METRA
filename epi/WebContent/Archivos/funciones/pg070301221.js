...
*/
    ...
    if (form.hdBoton.value != 'Cancelar') {
     ...
     if ((form.hdBoton.value == 'Guardar') || (form.hdBoton.value == 'GuardarA')){
      ...
      if (form.lCuantCual && (form.lCuantCual[1].checked || form.lCuantCual[0].checked) ){
         if (form.lCuantCual[1].checked){ 
           //alert("no. 0");
           if (form.dCorte2)
             cVMsg = cVMsg + fSinValor(form.dCorte2,4,'Corte:', true);
           if (form.dCorteNeg2)
             cVMsg = cVMsg + fSinValor(form.dCorteNeg2,4,'Corte Negativo:', true);
           if (form.dCortePost)
             cVMsg = cVMsg + fSinValor(form.dCortePost,4,'Corte Positivo', true);
         }else
         if (form.lCuantCual[0].checked){
           //alert("no. 1");
           if (form.dCorte1)
             cVMsg = cVMsg + fSinValor(form.dCorte1,4,'Corte:', true);
           if (form.dCorteNeg1)
             cVMsg = cVMsg + fSinValor(form.dCorteNeg1,4,'Corte Negativo:', true);
           if (form.cControles)
             cVMsg = cVMsg + fSinValor(form.cControles,0,'Controles:', true);
           if (form.dMargenError)
            cVMsg = cVMsg + fSinValor(form.dMargenError,4,'Margen de Error:', true);
         }
      }
      else
          cVMsg = cVMsg + "\n - Debe Seleccionar algún Corte (CG/EM o INMUNOENSAYO ).\n"

...
<html>
<%
  pg070801021CFG  clsConfig = new pg070801021CFG();                 // modificar

  TEntorno    vEntorno      = new TEntorno();
  vEntorno.setNumModulo(07);
  vEntorno.setOnLoad("fOnLoad();");
  vEntorno.setNombrePagina("pg070801021.jsp");                    // modificar
  vEntorno.setArchFuncs("FValida");
  vEntorno.setArchTooltips("07_Tooltips");
  vEntorno.setMetodoForm("POST");
  vEntorno.setActionForm("pg070801021.jsp\" target=\"FRMCuerpo"); // modificar
...
<script language="JavaScript">
  ...
  //  al directorio funciones de wwwrooting en un archivo con extensión js (vg. pg070801021.js)
...
<form method="<%= vEntorno.getMetodoForm() %>" action="<%= vEntorno.getActionForm() %>">
  ...
  <%
     if (bs != null){ // Agregar tanto Hds como variables dependiendo del campo llave.
       ...
       cClave  = ""+bs.getFieldValue("iCveFamilia", "");
     }
  %>
  ...
  <tr><td><input type="hidden" name="hdBoton" value="">
                          ...
                          <table border="1" class="ETablaInfo" align="center" cellspacing="0" cellpadding="3"><% // Inicio de Datos %>
                              <td colspan="6" class="ETablaT">Familia de Artículos
                              </td>

            <tr>
              <td class="ETablaTR">Tipo de Artículo:</td>
...
//**************************
              ...
              Vector vcEQMTpoArticulo = new Vector();
              if(request.getParameter("hdBoton").compareTo("Guardar")==0 || request.getParameter("hdBoton").compareTo("GuardarA")==0 || request.getParameter("hdBoton").compareTo("Cancelar")==0){
                out.print("<select name=\"iCveTpoArticulo\" size=\"1\" OnChange=\"document.forms[0].target='FRMDatos';document.forms[0].hdBoton.value='Primero';document.forms[0].submit();\">");
                vcEQMTpoArticulo = dEQMTpoArticulo.FindByAll("", " order by iCveTpoArticulo ");
                if (vcEQMTpoArticulo.size() > 0){
                  for (int i = 0; i < vcEQMTpoArticulo.size(); i++){
                    vEQMTpoArticulo = (TVALMTpoArticulo)vcEQMTpoArticulo.get(i);
                    if (request.getParameter("hdTpoArticulo")!=null && request.getParameter("hdTpoArticulo").compareToIgnoreCase(new Integer(vEQMTpoArticulo.getICveTpoArticulo()).toString())==0)
                      out.print("<option value = " + vEQMTpoArticulo.getICveTpoArticulo() + " selected>" + vEQMTpoArticulo.getCDscBreve() + "</option>");
                    else
                      out.print("<option value = " + vEQMTpoArticulo.getICveTpoArticulo() + ">" + vEQMTpoArticulo.getCDscBreve() + "</option>");
                  }
                }
                else{
                ...
                if(request.getParameter("hdBoton").compareTo("Nuevo")==0 || request.getParameter("hdBoton").compareTo("Modificar")==0)
                  out.print("<select name=\"iCveTpoArticulo\" disabled>");
                else
                  out.print("<select name=\"iCveTpoArticulo\" size=\"1\" OnChange=\"document.forms[0].target='FRMDatos';document.forms[0].hdBoton.value='Primero';document.forms[0].submit();\">");
                vcEQMTpoArticulo = dEQMTpoArticulo.FindByAll("", " order by iCveTpoArticulo ");
                if (vcEQMTpoArticulo.size() > 0){
                  ...
                  for (int i = 0; i < vcEQMTpoArticulo.size(); i++){
                    vEQMTpoArticulo = (TVALMTpoArticulo)vcEQMTpoArticulo.get(i);
                    if (request.getParameter("iCveTpoArticulo")!=null && request.getParameter("iCveTpoArticulo").compareToIgnoreCase(new Integer(vEQMTpoArticulo.getICveTpoArticulo()).toString())==0)
                      out.print("<option value = " + vEQMTpoArticulo.getICveTpoArticulo() + " selected>" + vEQMTpoArticulo.getCDscBreve() + "</option>");
                    else
                      out.print("<option value = " + vEQMTpoArticulo.getICveTpoArticulo() + ">" + vEQMTpoArticulo.getCDscBreve() + "</option>");
                  }
...
//************************
                               ...
                               if (lNuevo){ // Modificar de acuerdo al catálogo específico
                                 ...
                                 out.println("</tr>");
                                 out.println("<tr>");
                                 out.print(vEti.EtiCampo("EEtiqueta", "Descripción:", "ECampo", "text", 100, 100, "cDscFamilia", "", 3, "", "fMayus(this);", true, true, lCaptura));
                                 out.println("</tr>");
                                 out.println("<tr>");
                                 out.print(vEti.EtiCampo("EEtiqueta", "Descripción Breve:", "ECampo", "text", 100, 50, "cDscBreve", "", 3, "", "fMayus(this);", true, true, lCaptura));
                                 out.println("</tr>");
                                 out.println("<tr>");
                                 ...
                                 if (bs != null){
                                   out.println("<tr>");
                                   out.print(vEti.EtiCampo("EEtiqueta", "Clave:", "ECampo", "text", 4, 4, "iCveFamilia", ""+bs.getFieldValue("iCveFamilia", "&nbsp;"), 3, "", "", true, true, false));
                                   out.println("</tr>");
                                   out.println("<tr>");
                                   out.print(vEti.EtiCampo("EEtiqueta", "Descripción:", "ECampo", "text", 100, 100, "cDscFamilia", ""+bs.getFieldValue("cDscFamilia", "&nbsp;"), 3, "", "fMayus(this);", true, true, lCaptura));
                                   out.println("</tr>");
                                   out.println("<tr>");
                                   out.print(vEti.EtiCampo("EEtiqueta", "Descripción Breve:", "ECampo", "text", 100, 50, "cDscBreve", ""+bs.getFieldValue("cDscBreve", "&nbsp;"), 3, "", "fMayus(this);", true, true, lCaptura));
                                   out.println("</tr>");

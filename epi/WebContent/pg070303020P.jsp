...
<%@ page import="gob.sct.medprev.dao.TDExamenCualita" %>
<%@ page import="gob.sct.medprev.vo.TVExamenCualita" %>
<html>
  ...
  if (request.getParameter("opc") != null){
     ...
     switch(new Integer(request.getParameter("opc")).intValue()) {
        case 1:
           ...
           vExamenCualita.setIAnio(new Integer(request.getParameter("val1")).intValue());
           vExamenCualita.setICveLaboratorio(new Integer(request.getParameter("val2")).intValue());
           vExCuali = dExamenCualita.FindByLoteCualita(vExamenCualita);
           out.println("form = window.parent.parent.FRMCuerpo.FRMDatos.document.forms[0];");
           out.println("form." + ObjetoDes + ".length=1;");
           out.println("form." + ObjetoDes + "[0].text=" + "\"" + "Lote Nuevo..." + "\"" + ";");
           out.println("form." + ObjetoDes + "[0].value=" + "\"0\"" + ";");

           for (int i = 0; i < vExCuali.size(); i++){
              j = j + 1;
              vExamenCualita = (TVExamenCualita)vExCuali.get(i);
              out.println("i=form." + ObjetoDes + ".length + 1;");
              out.println("form." + ObjetoDes + ".length=i;");
              out.println("form." + ObjetoDes + "[" + j  + "].text=" + "\"" + vExamenCualita.getICveLoteCualita() + "\"" +";");
              out.println("form." + ObjetoDes + "[" + j + "].value="+ "\"" + vExamenCualita.getICveLoteCualita() + "\"" +";");
           }
        break;
        case 2:
        break;

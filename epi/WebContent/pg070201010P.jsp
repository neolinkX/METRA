...
<%
  ...
  String cTitulo = "";
  if (request.getParameter("hdTitulo") != null)
    cTitulo = "" + request.getParameter("hdTitulo");
  if(cTitulo.compareToIgnoreCase("") == 0)
     cTitulo = "";
  else
  ...
  if (request.getParameter("opc") != null){
     ...
     switch(new Integer(request.getParameter("opc")).intValue()) {
        ...
        case 2:
           ...
           vcPuesto = dGRLPuesto.FindByAll(request.getParameter("val2").toString(),request.getParameter("val1").toString());
           out.println("form = window.parent.parent.FRMCuerpo.FRMDatos.document.forms[0];");
           if (vcPuesto.size() > 0){
           out.println("form." + ObjetoDes + ".length=1;");
           out.println("form." + ObjetoDes + "[0].text=" + "\"" + "Seleccione..." + "\"" + ";");

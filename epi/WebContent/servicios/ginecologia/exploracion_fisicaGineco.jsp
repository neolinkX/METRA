...
<%@ page import="gob.sct.medprev.pg070101010CFG"%>
...
TParametro vParametros = new TParametro(vEntorno.getNumModuloStr());
	  pg070101010CFG  clsConfig = new pg070101010CFG();                 // modificar
	boolean lDetalle = clsConfig.getLPagina("pg070101010.jsp");
	clsConfig.verAcceso2();
	
	String CodigoValida = "";
	session.setAttribute("servicio", "71");
	String cRama = "12";
...
<%@ include file="/persona/datosServicios.jsp"%>
   ...
   <form id="frm_interrogatorio<%= cRama%>" name="frm_interrogatorio" class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed">
         ...
         <center>
               ...
               <span class="m-switch m-switch--sm m-switch--icon">
               <label> 
						<!--<input type="text" class="form-control m-input"	id="<%=CampoFinal%>" name="<%=CampoFinal%>" maxlength="200"	>  --> 
							<textarea maxlength="200" class="form-control m-input m-input--air" id="<%=CampoFinal%>" name="<%=CampoFinal%>" rows="2" ></textarea>
	...
	<%
            ...
            <label for="int_fiebre" class="col-3 col-form-label"> <%=vEXPResultados.getCPregunta()%>
               ...
               <label> 
						<!--<input type="text" class="form-control m-input"	id="<%=CampoFinal%>" name="<%=CampoFinal%>" maxlength="200"	>  --> 
							<textarea maxlength="200" class="form-control m-input m-input--air" id="<%=CampoFinal%>" name="<%=CampoFinal%>" rows="1" data-number></textarea>
...
function guarda_signos_vitales(){
  ...
  modalConfirm(function (){
    $.ajax({
        ...
        type: 'POST',
        data: $("#frm_interrogatorio<%= cRama%>").serialize(),
        dataType: 'json',
...
/*$("[data-number_dec]").inputmask({
 ...
 $("#frm_interrogatorio<%= cRama%>").validate({
	  rules: {
<%=CodigoValida%>
	  },

...
<html>
<%
  pg071008010CFG  clsConfig = new pg071008010CFG();                 // modificar
  ...
  vEntorno.setActionForm("pg071008010.jsp\" target=\"FRMCuerpo"); // modificar
...
<form method="<%= vEntorno.getMetodoForm() %>" action="<%= vEntorno.getActionForm() %>">
  ...
  <tr><td><input type="hidden" name="hdBoton" value="">
                          ...
                          <table border="1" class="ETablaInfo" align="center" cellspacing="0" cellpadding="3"><% // Inicio de Datos %>
                              <td colspan="4" class="ETablaT">Asignación de Modos de transporte a procesos
                              </td>
              ...
              out.print("<tr>");
                ...
                out.print("<select name=\"iCveModulo\" OnChange=\"llenaSLT(2,this.value,'','',document.forms[0].ICVEPROCESO);\">");
                ...
                out.print("</select></td>");
                     ...
                     out.print("<tr><td class='ETablaT'  colspan='4'>Proceso al que seran asignados estos modos de transporte</td></tr>");
                   // out.print("<tr><td class='EEtiqueta'>Selecciona el Proceso</td><td colspan='3'><select name='ICVEPROCESO' id='ICVEPROCESO'>");
                      out.print("<tr><td class='EEtiqueta'>Selecciona el Proceso</td><td colspan='3'><select name='ICVEPROCESO' id='ICVEPROCESO' OnChange=\"document.forms[0].target='FRMDatos';document.forms[0].hdBoton.value='Primero';document.forms[0].submit();\">");

                if (bs != null && request.getParameter("ICVEPROCESO")!=null  && request.getParameter("iCveUniMed")!=null && Integer.parseInt(request.getParameter("iCveUniMed"))>0 && request.getParameter("iCveModulo")!=null && Integer.parseInt(request.getParameter("iCveModulo"))>0){
                     TDGRLProceso vGRLProceso = new TDGRLProceso();
                      Vector vcGRLProceso = new Vector();
                      vcGRLProceso = vGRLProceso.FindByAll(" WHERE LACTIVO = 1", " ORDER BY ICVEPROCESO");
                      if (vcGRLProceso.size() > 0){
                        out.print("<option value = 0>Seleccione...</option>");
                        for (int i = 0; i < vcGRLProceso.size(); i++){
                          TVGRLProceso GRLProceso = (TVGRLProceso)vcGRLProceso.get(i);
                         if(GRLProceso.getICveProceso()==Integer.parseInt(request.getParameter("ICVEPROCESO")) )
                             out.print("<option value = " + GRLProceso.getICveProceso() + " selected>" + GRLProceso.getCDscProceso() + "</option>");
                          else
                                  out.print("<option value = " + GRLProceso.getICveProceso() + ">" + GRLProceso.getCDscProceso() + "</option>");
                        }
                      }else{
                        out.print("<option value = 0>Datos no disponibles</option>");
                      }
...
out.print("</select></td><td>&nbsp;</td></tr>");
                              ...
                              out.print("</tr>");
                                 if (bs != null && request.getParameter("iCveUniMed")!=null && Integer.parseInt(request.getParameter("iCveUniMed"))>0 && request.getParameter("iCveModulo")!=null && Integer.parseInt(request.getParameter("iCveModulo"))>0){
                                   ...
                                   while(bs.nextRow()){
                                     out.print("<tr>");
                                       out.print("<td class='ETablaR'>" + bs.getFieldValue("iCveMdoTrans") + "</td>");
                                       out.print("<td class='ETabla' colspan='2'>" + bs.getFieldValue("cDscMdoTrans") + "</td>");
                                       ...
                                       if(request.getParameter("iCveUniMed")!=null &&  request.getParameter("iCveUniMed").compareTo("")!=0 && request.getParameter("iCveModulo")!=null && request.getParameter("iCveModulo").compareTo("")!=0)
                                         vcGRLServUM = dGRLServUM.FindByUMMDPR(" where  a. ICVEMDOTRANS = b.ICVEMDOTRANS AND a.iCveUniMed = " + request.getParameter("iCveUniMed") + " and a.iCveModulo = " + request.getParameter("iCveModulo")+
                                                 " and a.ICVEPROCESO = " + request.getParameter("ICVEPROCESO")+" and a.ICVEMDOTRANS = " +bs.getFieldValue("iCveMdoTrans"),"");
                                       if(vcGRLServUM.size()>0){
                                         vGRLServUM = (TVGRLProcesoMDOT)vcGRLServUM.get(0);
                                         out.print("<td align='center'><input type='checkbox' value='" + bs.getFieldValue("iCveMdoTrans") + "' name='chk" + iObjeto + "' checked></td>");
                                       }
                                       else{
                                         out.print("<td align='center'><input type='checkbox' value='" + bs.getFieldValue("iCveMdoTrans") + "' name='chk" + iObjeto + "'></td>");
                                       }
                                       out.print("<input type='hidden' value='" + bs.getFieldValue("iCveMdoTrans") + "' name='hdChk" + iObjeto + "'></td>");
                                     out.print("</tr>");
                                     iObjeto++;
                                   }

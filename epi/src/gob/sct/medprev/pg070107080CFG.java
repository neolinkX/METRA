 ...
 */
public class pg070107080CFG extends CFGListBase2 {
	public pg070107080CFG() {
	...
	public void fillVector() {
		...
		try {
			...
			if (request.getParameter("hdICvePersonal") != null) {
				/*
				 * vDespliega =
				 * dEXPExamAplica.FindByAll("where EXPExamAplica.iCveExpediente = "
				 * + request.getParameter("hdICvePersonal") +
				 * " and EXPExamAplica.iNumExamen = (SELECT MAX(INUMEXAMEN) FROM EXPEXAMAPLICA WHERE ICVEEXPEDIENTE = "
				 * + request.getParameter("hdICvePersonal") +
				 * " AND ICVEPROCESO = 1)");
				 */
				vDespliega = dEXPExamAplica
						.FindByAll("where EXPExamAplica.iCveExpediente = "
								+ request.getParameter("hdICvePersonal")
								+ " AND EXPExamAplica.ICVEPROCESO < 3 ");

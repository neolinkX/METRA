 ...
 *      <p>
 *      <img src='pg070403070CFG.png'>
 */
public class pg070403070CFG extends CFGListBase2 {
	...
	public void mainBlock() {
		...
		if (request.getParameter("iCveTpoCausa") != null)
			if (request.getParameter("iCveTpoCausa").toString()
					.compareToIgnoreCase("") != 0)
				ivTpoCausa = new Integer(request.getParameter("iCveTpoCausa"))
						.intValue();
	...
	public Vector getValores(int ivAnio, int ivMdoTransporte, int ivTpoCausa) {
		TDINVRegistro DINVRegistro = new TDINVRegistro();

		try {
			vAccidentes = DINVRegistro.CountByAnioCausa(ivAnio,
					ivMdoTransporte, ivTpoCausa);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		return vAccidentes;
	...
	public void fillVector() {
		TDINVCausa DINVCausa = new TDINVCausa();
		try {
			String cWhere = "";
			String cOrderBy = "";
			...
			if (cOrden.compareTo("") != 0) {
				...
				cOrderBy = " order by cDscCausa";

			cWhere = " iCveTpoCausa = " + ivTpoCausa;

			vDespliega = DINVCausa.FindByAll(cWhere, cOrderBy);
	...
	public void Enviar() {
		...
		try {
			vEntidad = DINVCausa.FindByAll(" iCveTpoCausa = " + ivTpoCausa,
					" order by cDscCausa");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		...
		vRegistro = this.getValores(ivAnio, ivMdoTransporte, ivTpoCausa);
		Rep1.comEligeHoja(1);
		...
		if (!vEntidad.isEmpty()) {
			for (int i = 0; i < vEntidad.size(); i++) {
				...
				VINVCausa = (TVINVCausa) vEntidad.get(i);
				ivEnero = 0;
				ivFebrero = 0;
				ivMarzo = 0;
				ivAbril = 0;
				...
				for (int j = 0; j < vRegistro.size(); j++) {
					...
					if (VINVRegistro.getICveCausa() == VINVCausa.getICveCausa()) {
				...
				Rep1.comDespliega("C", ivInicial, "" + VINVCausa.getCDscCausa());

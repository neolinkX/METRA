...
public class pg070101073CFG extends CFGListBase2 {
	int iCveSistema = 07;
			...
			for (int i = 0; i < vDespliega2.size(); i++) {
				...
				if (request.getParameter("TBXGral-"
					...
					if (new Integer(request.getParameter("TBXGral-"
						...
						// Eliminaci�n de los Registros en caso de Tener
						// previos.
						try {
							DMEDSintExamen
									.deleteCustomWhere(
											null,
											" delete from MEDSintExamenTMP "
													+ " where iCveProceso  =      "
													+ ivProceso
													+ "   and iCveMotivo   =      "
													+ ivMotivo
						...
						try {
							DMEDSintExamen.insert2(null, VMEDSintExamen);
						} catch (Exception ex) {
							ex.printStackTrace();
						}

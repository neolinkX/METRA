 ...
 *      
 */
public class pg070104003CFG extends CFGListBase2 {
	public pg070104003CFG() {
...
*/
			//vPerDat = new TDEXPExamAplica().findUserV2(iCvePersonal);
			vPerDat = new TDEXPExamAplicaExt1().findUserV2(iCvePersonal);
		} catch (Exception ex) {
			error("findUser", ex);
		}
		return vPerDat;
	 ...
	 */
	public TVPERDatos findExpToEliminaBiometricoNAS() {
		TVPERDatos vPerDat = null;
		try {
			vPerDat = new TDEXPExamAplicaExt1().findExpToEliminaBiometricoNAS(this.getInputs());
		} catch (Exception ex) {
			error("findUser", ex);
		}
		return vPerDat;

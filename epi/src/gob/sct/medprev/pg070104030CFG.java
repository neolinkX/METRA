...
/**
 * * Clase de configuracion para Handler de acciones para la p�gina pg070104030
 * <p>
 ...
 */
public class pg070104030CFG extends CFGListBase2 {
	...
	public void fillVector() {
		...
		/*
		 * Si el proceso es por un s�lo m�dico, deben obtenerse todas las ramas
		 * del servicio e iterar para procesar cada una de ellas.
		 * 
		 * Si el proceso es por varios m�dicos, se debe tomar la rama que viene
		 * seleccionada y procesar s�lo esa rama.
		 */
	...
	private void actualizar(Connection conn) throws DAOException {
		...
		if (exp > -1 && exm > -1 && srv > -1 && ram > -1) {
			...
			if (vRama.size() > 0
				...
				// log("--antes de actualizar ramas y servicios, los parametros son: "
				// + param.toString());
				...
				if (rama != null) { // validar que existe el registro
					...
					if (rama.getDtInicio() == null) {
						rama.setDtInicio(dtFecha); // poner fecha de inicio s�lo
													// si no tiene
					}
	...
	private void terminarServicio() throws DAOException {
		...
		if (servicio != null) {
			...
			if (servicio.getDtInicio() == null) {
				...
				servicio.setDtInicio(dtFecha); // poner fecha de inicio s�lo si
												// no tiene
			}
	...
	private HashBeanInterface llenarRegistro(HashBeanInterface value)
		...
		if (value instanceof TVEXPRama) { // si recibe una rama
			 ...
			 * rama.getICveExpediente() + ""); filtro.put("iNumExamen",
			 * rama.getINumExamen() + ""); filtro.put("iCveServicio",
			 * rama.getICveServicio() + ""); filtro.put("iCveRama",
			 * rama.getICveRama() + "");
			 */
			String filtros = "";
	 ...
	 * 
	 * @return un value object con el registro de la rama, o null si no hay m�s
	 *         registros.
	 * @author Romeo Sanchez
	 */
	...
	/**
	 * Indica si existen m�s ramas sin concluir para el servicio proces�ndose
	 * actualmente.
	 * 
	...
	/**
	 * Método que devuelve la lista de unidades m�dicas v�lidas
	 * 
	...
	/**
	 * Método que devuelve todos los m�dulos correspondientes a la unidad m�dica
	 * indicada en el par�metro "iCveUniMed" del request de HTTP.
	 * 
	...
	/**
	 * Método que devuelve todos los m�dulos correspondientes a la unidad m�dica
	 * indicada en el par�metro que recibe el Método.
	 * 
	 * @return un Vector con los m�dulos
	 */
	...
	/**
	 * Método que devuelve la descripci�n correspondiente al n�mero de proceso.
	 * 
	 * @param param
	...
	/**
	 * Método que convierte la colecci�n de servicios recibida como HashMap en
	 * un Vector
	 * 
	 * @param hm

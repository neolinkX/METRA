 ...
 *      <p>
 *      <img src='pg070104002CFG.png'>
 */
public class pg070104002CFG extends CFGListBase2 {
	public pg070104002CFG() {
	...
	public TVPERDatos findExpediente() {
		...
		try {
			 ...
			 * if (cCveExpediente == null){ cCveExpediente = "0"; } int
			 * iCveExpediente=Integer.parseInt(cCveExpediente);
			 */
			vPerDat = new TDEXPExamAplica().findUserbyExp(this.getInputs());
		} catch (Exception ex) {
			error("findUser", ex);
		}
		return vPerDat;
	...
	public Object getInputs() throws CFGException {
		...
		try {
			...
			// System.out.println("Concluido = "+iCampo);
	...
	///*********** Eliminando carga de Sintomas 2014-03-25 *******************
					//*********** Solo se cargaran los sintmas del servicio Laboratorio de analisis clinicos 2014-04-02 *******************
    ...
    ///***********************************************************************					
							...
							if (vcGrlUniMed.size() > 0) {
								...
								if (vGRLUniMed.getLPago() != 0) {
									...
									cWhere2.append(" where S.iCveProceso  = 1"
											...
											+ " and ( SN.cGenero = 'A' or SN.cGenero = '"
											+ cGenero + "')"
											+ " and S.iCveServicio = 2");
									
									SQLUpdate = "UPDATE EXPSERVICIO SET " +
											"TSINICIO = (CURRENT TIMESTAMP) " +
											"WHERE ICVEEXPEDIENTE = "+request.getParameter("iCveExpediente")+" AND " +
											"INUMEXAMEN = "+request.getParameter("iNumExamen")+" AND ICVESERVICIO = 2";

			///*********** Eliminando carga de Sintomas 2014-03-25 *******************
									//*********** Solo se cargaran los sintmas del servicio Laboratorio de analisis clinicos 2014-04-02 *******************
									vcMEDSintExamen = dMEDSintExamen.FindAllExam2(cWhere2);
		    ///***********************************************************************
	    ...
	    ///***********************************************************************************************									
									Vector<Object> vacio = new Vector<Object>();
									...
									int sint = dMEDSintExamen.FindAllExam3Count(cWhere2);
...
/*
									...
									if ((sint > exasint) || (serv > exaserv) || (rama > exarama)) {*/
									if ((serv > exaserv) || (rama > exarama)) {
									 //System.out.println("Se detecto Vale de Servicio mal generado");
										dEXPExamAplica.GeneraExamen(vcEXPServicio,
												vcEXPRama, vacio);
									...
									////Nueva implementacion solo carga de LAboratorio
	    ...
	    ///***********************************************************************************************										
						...
						if (vcGrlUniMed.size() > 0) {
							...
							dEXPExamAplica.updateValeServicio(null, this.getInputs());
							// }
						}
					}
	...
	public String Guardar2(String exp, String exa, String cGenero, String proceso) {
		//System.out.println("Guardar2");
				/*
				 * Calcula Fecha Actual
				 */
		String respuesta ="";
				...
				try {
					...
					boolean fAccepted = false;
					//Object objjsp = this.getInputs();
					Object objjsp = this.getInputsDos(exp,exa);
    ...
    ///***********************************************************************					
					...
					vcExpExamCat = dEXPExamAplica
							.FindExpExamCat(null, this.getInputsDos(exp,exa));
					vcExamAp = dEXPExamAplica
							.FindByAll(" where EXPExamAplica.iCveExpediente = "
									+ exp
									+ " and EXPExamAplica.iNumExamen = "
									+ exa);
					System.out.println("vcExpExamCat="+vcExpExamCat.size());
					System.out.println("vcExamAp="+vcExamAp.size());
					if (vcExamAp.size() > 0
							&& ((TVEXPExamAplica) (vcExamAp.get(0))).getLIniciado() == 0) {
						if (vcExpExamCat.size() > 0) {
							...
							if (vcGrlUniMed.size() > 0) {
								...
								if (vGRLUniMed.getLPago() != 0) {
									...
									if (vcGrlMotivo.size() > 0) {
										for (int x = 0; x < vcGrlMotivo.size(); x++) {
											vGRLMotivo = (TVGRLMotivo) vcGrlMotivo
													.get(x);
											if (vGRLMotivo.getLPago() == 0) {
												 System.out.println( "NEGATIVO - Queda pendiente el WEBService - Los Motivos y sus Pagos");
												fAccepted = true; // Esto es por el
																	// momento DEBE SER
																	// FALSE
											} else {
									...
									SQLUpdate = "UPDATE EXPSERVICIO SET " +
											...
											"INUMEXAMEN = "+exa+" AND ICVESERVICIO = 2";
		    ...
		    ///***********************************************************************
									...
									for (int i = 0; i < vcMEDSintExamenServ.size(); i++) {												
										...
										vEXPServicio = new TVEXPServicio();
										vEXPServicio
												.setICveExpediente(Integer.parseInt(exp));
										vEXPServicio.setINumExamen(Integer
												.parseInt(exa));
									...
									for (int i = 0; i < vcMEDSintExamenRama.size(); i++) {
										vMEDSintExamen = (TVMEDSintExamen) vcMEDSintExamenRama
												.get(i);
										vEXPRama = new TVEXPRama();
										vEXPRama.setICveExpediente(Integer.parseInt(exp));
										vEXPRama.setINumExamen(Integer.parseInt(exa));
										vEXPRama.setICveServicio(vMEDSintExamen
												.getICveServicio());
										vEXPRama.setICveRama(vMEDSintExamen
												.getICveRama());
									...
									for (int i = 0; i < vcMEDSintExamen.size(); i++) {
										...
										vEXPResultado
												.setICveExpediente(Integer.parseInt(
														...
														10));
										vEXPResultado
												.setINumExamen(Integer.parseInt(exa, 10));
										vEXPResultado.setICveServicio(vMEDSintExamen
												.getICveServicio());
										vEXPResultado.setICveRama(vMEDSintExamen
												.getICveRama());
	    ...
	    ///***********************************************************************************************										
						...
						if (vcGrlUniMed.size() > 0) {
							// if (vGRLUniMed.getLPago() != 0) {
							dEXPExamAplica.updateArchivoDos(null, this.getInputsDos(exp,exa));
							dEXPExamAplica.updateValeServicio(null, this.getInputsDos(exp,exa));
							// }
						}
					}
					// dEXPExamAplica.update(null, this.getInputs());
					respuesta="El Proceso EPI se inicio Correctamente";
					respuesta = "success";
				} catch (Exception ex) {
					vErrores.acumulaError("", 14, "");
					error("Error al insertar el registro", ex);
					respuesta="No fue posible iniciar el proceso EPI";
				} finally {
					super.Guardar();
					return respuesta;
				}
	 ...
	 * Metodo getInputs
	 */
	public Object getInputsDos(String exp, String exa) throws CFGException {
		String cCampo;
		int iCampo;
		float fCampo;
		java.sql.Date dtCampo;

 ...
 *      <p>
 *      <img src='pg070403010CFG.png'>
 */
public class pg070403010CFG extends CFGListBase2 {
	...
	public Vector getValores(int ivAnio, int ivMdoTransporte) {
		...
		try {
			vAccidentes = DINVRegistro.CountByAnioUniMed(ivAnio,
					ivMdoTransporte);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public void Enviar() {
		...
		try {
			vEntidad = DGRLUniMed.FindByAll("", "order by cDscUniMed");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		...
		if (!vEntidad.isEmpty()) {
			for (int i = 0; i < vEntidad.size(); i++) {
				...
				VGRLUniMed = (TVGRLUniMed) vEntidad.get(i);
				ivEnero = 0;
				ivFebrero = 0;
				ivMarzo = 0;
				ivAbril = 0;
				...
				for (int j = 0; j < vRegistro.size(); j++) {
					...
					if (VINVRegistro.getICveUniMed() == VGRLUniMed
							.getICveUniMed()) {

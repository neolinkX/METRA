 ...
 */
public class pg070104001TMPCFG extends CFGListBase2 {
	public pg070104001TMPCFG() {
		vParametros = new TParametro("07");
		DeleteAction = "BorrarB";
	}

	public void generaValeDeServicioNuevamente() {
	...
	public void Guardar() {
		...
		try {
			 ...
			 */
		     ...
		     //// Ya no se guardara la fecha de aplicacion en la liberacion del vale de servicio ASL 2016-05-19
			...
			TDMEDSintExamenTMP dMEDSintExamen = new TDMEDSintExamenTMP();
		...
		//// Ya no se guardara la fecha de aplicacion en la liberacion del vale de servicio por tal motivo se comento la siguiente validacion ASL 2016-05-19
			//if (vcExamAp.size() > 0	&& ((TVEXPExamAplica) (vcExamAp.get(0))).getLIniciado() == 0) {
			if (vcExamAp.size() > 0) {
				...
				mod.setiCveExpediente(request.getParameter("iCveExpediente"));
				mod.setiNumExamen(request.getParameter("iNumExamen"));
				mod.setOperacion("5");
				mod.setDescripcion("Liberación del Vale de Servicios");
				...
				if (vcGrlUniMed.size() > 0) {
					// if (vGRLUniMed.getLPago() != 0) {
					dEXPExamAplica.updateLiberaVale(null, this.getInputs());
					// }
				}
			}

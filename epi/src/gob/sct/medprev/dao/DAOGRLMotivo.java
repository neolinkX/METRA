...
public class DAOGRLMotivo extends DAOBase {
	...
	public Vector FindByAll() throws DAOException {
		...
		try {
			...
			cSQL = "select " + "iCveMotivo," + "iCveProceso," + "cDscMotivo"
					+ " from GRLMotivo order by iCveMotivo";
			pstmt = conn.prepareStatement(cSQL);
			rset = pstmt.executeQuery();
			while (rset.next()) {
				lSuccess = true;
				vMotivo = new TVMotivo();
				vMotivo.setICveMotivo(rset.getInt(1));
				vMotivo.setICveProceso(rset.getInt(2));
				vMotivo.setCDscMotivo(rset.getString(3));
				vcOTOXSustancia.addElement(vMotivo);
			}
		} catch (Exception ex) {
			warn("FindByAll", ex);
	...
	/**
	 * Metodo Find By Proceso Importante: Este metodo se usa en el Aplication
	 * Cache, en donde se extraen los motivos dependiendo de los procesos.
	 */
	public Vector FindByProceso(int iCveProceso) throws DAOException {
		DbConnection dbConn = null;
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		...
		try {
			...
			conn.setTransactionIsolation(2);
			int count;
			cSQL = " select " + " iCveMotivo," + " iCveProceso,"
					+ " cDscMotivo" + " from GRLMotivo where iCveProceso = "
					+ iCveProceso + "  and GRLMotivo.lActivo = 1 "
					+ " order by cDscMotivo";
			pstmt = conn.prepareStatement(cSQL);
			rset = pstmt.executeQuery();
			while (rset.next()) {
	...
	public boolean findCollection(String cCollName, String cKey) {
		if (cCollName.compareTo("GRLMotivo") == 0) {
			try {
				StringTokenizer stKey = new StringTokenizer(cKey, "|");
				oReturn = this.FindByProceso(Integer.parseInt(
						stKey.nextToken(), 10));
			} catch (Exception e) {

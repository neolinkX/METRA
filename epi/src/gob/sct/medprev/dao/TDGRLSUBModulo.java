...
public class TDGRLSUBModulo extends DAOBase {
	private TParametro VParametros = new TParametro("07");
	private String dataSourceName = VParametros
			.getPropEspecifica("ConDBModulo");

	public TDGRLSUBModulo() {
	}
	...
	public Vector FindByAll(String cWhere) throws DAOException {
		...
		try {
			...
			String cSQL = "";
			TVGRLSUBModulo vGRLSUBModulo;
			conn.setAutoCommit(false);
			conn.setTransactionIsolation(2);
			int count;
			cSQL = "select " + "iCveUniMed," + "iCveModulo," + "iCveSubModulo," 
					+"cDscModulo " + 
					" from GRLSubModulo " + cWhere;

			//System.out.println(cSQL);
			pstmt = conn.prepareStatement(cSQL);
			rset = pstmt.executeQuery();
			while (rset.next()) {
				vGRLSUBModulo = new TVGRLSUBModulo();
				vGRLSUBModulo.setICveUniMed(rset.getInt(1));
				vGRLSUBModulo.setICveModulo(rset.getInt(2));
				vGRLSUBModulo.setICveSubModulo(rset.getInt(3));
				vGRLSUBModulo.setCDscModulo(rset.getString(4));
				vcGRLModulo.addElement(vGRLSUBModulo);
			}
		} catch (Exception ex) {
			warn("FindByAll", ex);

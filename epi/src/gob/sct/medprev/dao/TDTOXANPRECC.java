...
/**
 * <p>
 * Title: Data Acces Object de TOXANPRECC DAO
 * </p>
 * <p>
...
public class TDTOXANPRECC extends DAOBase {
	...
	public Vector FindByAll(String cvFiltro) throws DAOException {
		...
		try {
			...
			int count;
			cSQL = " select  "
					+ "toxanprecc.iCveSustancia,   "
					+ "toxanprecc.iCveCorte,    "
					+ "toxanprecc.lActivo,     "
					+ "toxanprecc.dtInicio,   "
					+ "toxanprecc.dtFin,   "
					+ "toxanprecc.iCveUsuAutoriza,            "
					+ "toxanprecc.dCorte,  "
					+ "toxanprecc.dMargenError,  "
					+ "(segusuario.cNombre || ' ' || segusuario.cappaterno || ' ' || segusuario.capmaterno) as cDscUsuAutoriza, "
					+ "toxsustancia.cDscSustancia "
					+ "from TOXAnPreCC  "
					+ "left join toxsustancia on toxsustancia.icvesustancia = toxanprecc.icvesustancia  "
					+ "left join segusuario on segusuario.icveusuario = toxanprecc.iCveUsuAutoriza   "
					+ cvFiltro + " ";
			...
			while (rset.next()) {
				...
				vTOXANPRECC.setiCveSustancia(new Integer(rset.getInt(1)));
				vTOXANPRECC.setiCveCorte(new Integer(rset.getInt(2)));
				vTOXANPRECC.setlActivo(new Integer(rset.getInt(3)));
				vTOXANPRECC.setdtInicio(rset.getDate(4));
				vTOXANPRECC.setdtFin(rset.getDate(5));
				vTOXANPRECC.setiCveUsuAutoriza(new Integer(rset.getInt(6)));
				vTOXANPRECC.setdCorte(new Double(rset.getDouble(7)));
				vTOXANPRECC.setdMargenError(new Double(rset.getDouble(8)));
				vTOXANPRECC.setCDscUsuAutoriza(rset.getString(9));
				vTOXANPRECC.setCDscSustancia(rset.getString(10));
				 ...
				 * vTOXCorteXSust.setiCveUsuAutoriza(new
				 * Integer(rset.getInt(6))); vTOXCorteXSust.setdCorte(new
				 * Double(rset.getDouble(7))); vTOXCorteXSust.setdCorteNeg(new
				 * Double(rset.getDouble(8))); vTOXCorteXSust.setdCortePost(new
				 * Double(rset.getDouble(9)));
				 * vTOXCorteXSust.setdMargenError(new
				 * Double(rset.getDouble(10)));
	...
	public Object insert(Connection conGral, Object obj) throws DAOException {
		...
		try {
			...
			conn.setAutoCommit(false);
			conn.setTransactionIsolation(2);
			cSQL = "select max(iCveCorte) from TOXANPRECC where iCveSustancia = ?";
			...
			cSQL = " insert into TOXANPRECC( " + " iCveSustancia, "
					+ " iCveCorte, " + " lActivo, " + " dtInicio, "
					+ " dtFin, " + " iCveUsuAutoriza, " + " dCorte, "
					+ " dMargenError," + " tIcorte"
					+ ") values(?,?,?,?,?,?,?,?,?) ";
			...
			if (vTOXCorteXSust.getlActivo().intValue() == 1
				...
				cSQL = "update TOXANPRECC set lActivo = 0 where iCveSustancia = ? and iCveCorte <> ? and dCortePost > 0";
			...
			if (vTOXCorteXSust.getlActivo().intValue() == 1
				...
				cSQL = "update TOXANPRECC set lActivo = 0 where iCveSustancia = ? and iCveCorte <> ? and dMargenError > 0";
				pstmtUpd = conn.prepareStatement(cSQL);
				pstmtUpd.setInt(1, vTOXCorteXSust.getiCveSustancia().intValue());
				pstmtUpd.setInt(2, vTOXCorteXSust.getiCveCorte().intValue());
				pstmtUpd.executeUpdate();
	...
	public Object update(Connection conGral, Object obj) throws DAOException {
		...
		try {
			...
			conn.setAutoCommit(false);
			conn.setTransactionIsolation(2);
			cSQL = "update TOXANPRECC" + " set lActivo= ?, " + "dtInicio= ?, "
					+ "dtFin= ?, " + "iCveUsuAutoriza= ?, " + "dCorte= ?, "
					+ "dMargenError= ?, " + "where iCveSustancia = ? "
					+ " and iCveCorte = ?";
			pstmt = conn.prepareStatement(cSQL);

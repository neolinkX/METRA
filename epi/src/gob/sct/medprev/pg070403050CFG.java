 ...
 *      <p>
 *      <img src='pg070403050CFG.png'>
 */
public class pg070403050CFG extends CFGListBase2 {
	...
	public Vector getValores(int ivAnio, int ivMdoTransporte) {
		...
		try {
			vAccidentes = DINVRegistro.CountByAnioServicioPrestado(ivAnio,
					ivMdoTransporte);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public void fillVector() {
		...
		try {
			...
			if (cOrden.compareTo("") != 0) {
				...
				cOrderBy = "order by cDscServPrestado";
	...
	public void Enviar() {
		...
		int ivIniHoja2 = 1; // Valor del Renglon Inicial del Reporte.
		int ivTotUniMed = 0;
		int ivTotGeneral = 0;

		// Obtenci�n de los Valores de los Estados.
		try {
			vEntidad = DCTRServPrestado.FindByAll("",
					"order by cDscServPrestado");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		...
		if (!vEntidad.isEmpty()) {
			for (int i = 0; i < vEntidad.size(); i++) {
				TVCTRServPrestado VCTRServPrestado = new TVCTRServPrestado();
				VCTRServPrestado = (TVCTRServPrestado) vEntidad.get(i);
				ivEnero = 0;
				ivFebrero = 0;
				ivMarzo = 0;
				ivAbril = 0;
				...
				for (int j = 0; j < vRegistro.size(); j++) {
					...
					if (VINVRegistro.getICveUniMed() == VCTRServPrestado
							.getICveServPrestado()) {
				...
				Rep1.comDespliega("C", ivInicial,
						"" + VCTRServPrestado.getCDscServPrestado());
				...
				Rep1.comEligeHoja(2);
				Rep1.comDespliega("A", ivIniHoja2,
						"" + VCTRServPrestado.getCDscServPrestado());
				Rep1.comDespliega("B", ivIniHoja2, "" + ivTotUniMed);

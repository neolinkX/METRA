 ...
 */
public class pg070104040CFG extends CFGCatBase2 {
	...
	public void Guardar() {
		...
		try {
			...
			vExamGenera.setICveExpediente(Integer.parseInt(request
					.getParameter("hdiCveExpediente")));
			vExamGenera.setINumExamen(Integer.parseInt(request
					.getParameter("hdiNumExamen")));
			vExamGenera.setICveProceso(Integer.parseInt(request
					.getParameter("hdToxProceso")));

			TVEXPExamAplica vExamAplica = new TVEXPExamAplica();
			TDPERDatos dPERDatos = new TDPERDatos();
			TVPERDatos vPERDatos = new TVPERDatos();
			vExamAplica.setICveExpediente(Integer.parseInt(request
					.getParameter("hdiCveExpediente")));
			vExamAplica.setICveUniMed(Integer.parseInt(request
					.getParameter("iCveUniMed")));
			vExamAplica.setICvePersonal(Integer.parseInt(request
					.getParameter("hdiCvePersonal")));
			vExamAplica.setICveModulo(Integer.parseInt(request
					.getParameter("iCveModulo")));
			vExamAplica.setDtSolicitado(dtFecha.TodaySQL());

			vPERDatos = dPERDatos.findUser(Integer.parseInt(request
					.getParameter("hdiCvePersonal")));

			vExamAplica.setICveNumEmpresa(vPERDatos.getICveNumEmpresa());
			vExamAplica.setICveUsuSolicita(Integer.parseInt(request
					.getParameter("iCveUsuCaptura")));

			cClave = (String) dPbaRapida.InsertEMOEPIINV(null,
					this.getInputs(), vcSustanciaApi, vExamGenera, vExamAplica);

 ...
 */
public class pg070101103CFG extends CFGListBase2 {
	public pg070101103CFG() {
		...
		try {
			...
			if (lAction.equalsIgnoreCase("Primero")
				...
				if (cCond.trim().length() == 0) {
					...
					cWhere += cCond + " ";
					vDespliega = dMEDREGSIN.FindByAllAlInf(cWhere);
				}
			}
			...
			if (lAction.equalsIgnoreCase("BorrarB")) {
				...
				dMEDREGSIN.delete(null, iCveServicio, iCveRama, iCveSintoma,
						" and RDINFO = 1", usuario.getICveusuario());
				/*
				dMEDREGSIN.delete(null, iCveServicio, iCveRama, iCveSintoma,
						" and RDINFO = 1");*/
				// dMEDAREG.delete(null, iCveServicio, iCveRama, iCveSintoma,
				// " and SintomaDep IS NOT NULL");
				// vDespliega = dMEDREGSIN.FindByAllAlInf(cWhere );
			}// fin BorrarB

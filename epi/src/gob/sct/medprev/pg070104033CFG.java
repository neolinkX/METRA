...
/**
 * * Clase de configuracion para Handler de acciones para la p�gina pg070104033
 * <p>
 ...
 *      <p>
 *      <img src='pg070104033.jsp.png'>
 */
public class pg070104033CFG extends CFGListBase2 {
	...
	public pg070104033CFG() {
		this.vParametros = new TParametro("07");
		cPaginas = "pg070105010.jsp|pg070104031.jsp"; // Ir a...
	}
	...
	public void fillVector() {
		...
		if (cAccion.equalsIgnoreCase("Guardar")) {
			...
			log("* para insertar: " + porInsertar.toString());
	...
	private boolean existe(HashBeanInterface obj) throws DAOException {
		...
		if (obj instanceof TVEXPServicio) {
			...
			// System.out.println("pg070104033CFG");
	...
	private void generar(Connection conn, Vector actualizar)
			throws DAOException {
		log("en generar() los parametros son: " + param.toString());
		...
		log("servicio buscado: " + servicio.toHashMap().toString());
		// insertar EXPServicio, si no existe
		if (!existe(servicio)) {
			log("no existe el servicio");
	...
	private HashBeanInterface llenarRegistro(HashBeanInterface value)
		...
		if (value instanceof TVEXPRama) { // si recibe una rama
			 ...
			 */
			String filtros = "";
			filtros = "iCveExpediente = " + rama.getICveExpediente()
					+ " and iNumExamen = " + rama.getINumExamen()
					+ " and iCveServicio = " + rama.getINumExamen()
					+ " and iCveRama= " + rama.getINumExamen() + " ";
			// System.out.println(filtros+"--1");
			try {
	...
	private Vector getTableValues() {
		...
		log("sintomas por insertar: " + sintomas);
		return sintomas;
	}

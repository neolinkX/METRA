 ...
 */
public class pg070101072CFG extends CFGCatBase2 {
	public pg070101072CFG() {
	...
	public void Guardar() {
		...
		try {
			...
			dMEDServExamen.delete2(null, this.getInputs());

			for (int j = 0; j < vSrvs.size(); j++) {
				vTVS = (TVMEDServicio) vSrvs.get(j);
				if (request.getParameter("General" + j) != null) {
					// dMEDServExamen.update2(null,this.getInputs(),Integer.parseInt((String)request.getParameter("iCveServicio"+j)),0);
					if (vMots.size() > 0) {
						for (int k = 0; k < vMots.size(); k++) {
							vTVMots = (TVGRLMotivo) vMots.get(k);
							dMEDServExamen.update2(null, this.getInputs(),
									Integer.parseInt((String) request
											.getParameter("iCveServicio" + j)),
									vTVMots.getICveMotivo());
						}
					} else {
						dMEDServExamen.update2(null, this.getInputs(), Integer
	...
	public void fillVector() {
		...
		try {
			...
			vDespliega = dMEDServExamen.FindByAll2(" wHere ");
			...
			if (lAction.equalsIgnoreCase("Primero")) {
				...
				cWhere += cCond + " " + cOrder;
				// vDespliega = dMEDServExamen.FindByAll2(cWhere);
				vDespliega = dMEDServExamen.FindByAll2(" WHEre ");
			}
			if (lAction.equalsIgnoreCase("Buscar")
				...
				if (cCond != null) {
					if (cCond.compareTo("null") == 0 || cCond.length() == 0) {
						...
						cCond = " WHERE a.iCveProceso = " + cCond + " and ";
					}
				} else {
					cCond = "";
				}

...
public class AdministradorContenidosServletVoucher extends HttpServlet {
	/**
	 * 
	 */
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		//System.out.println(VParametros.getPropEspecifica("RutaNASSM")+"myfile");
		//File archivo = File.createTempFile("myfile", ".tmp");
		File archivo = File.createTempFile("myfile", ".tmp", new File(VParametros.getPropEspecifica("RutaNASSM")));
		...
		try {
			try {
				FileItem fileItem = wrapper.getFileItem("FICHERO");
				try {
		...
		if (archivo.length() >= TamMaxImgServicios) {
			...
			if (archivo.length() > 10) {
				...
				resultado = content.subirArchivoNASV2(claveDocumento,obterneByteArchivos(archivo));//Agregado el 31 de Mayo 2016 AG SA
				// resultado = 0;				
			}else{
				resultado = 1;
			}
		...
		if (resultado == 0) {// si se subio el archivo correctamente
			...
			try {
				guardaReferenciaArchivo(claveDocumento, wrapper);
			...
			out.print("<%@ page language=\"java\" contentType=\"text/html; charset=ISO-8859-1\" pageEncoding=\"ISO-8859-1\"%>"
					...
					+ "<form method=\"get\" action=\"AdministradorContenidosServletVoucher\" enctype=\"multipart/form-data\">"
					+ "<br>"
	...
	public void guardaReferenciaArchivo(int idArchivo, FileUploadWrapper wrapper) throws NumberFormatException, DAOException {
		DBObject db = new DBObject();
		TDEXPMultas dEXPMulta = new TDEXPMultas(); 
		int iClaveMulta = 0;
		System.out.println("guardaReferenciaArchivo");
		System.out.println(wrapper.getParameter("iCveExpediente"));
		System.out.println(wrapper.getParameter("iCvePersonal"));
		iClaveMulta = dEXPMulta.MaximoMulta(Integer.parseInt(wrapper.getParameter("iCveExpediente").toString())) + 1;
		String SQL = "INSERT INTO EXPSERVARCHCM(" + "ICVEEXPEDIENTE, "
				...
				+ "CTIPODOCUMENTO" + ",LVALIDO, ICVEMULTA) " + "VALUES (" + ""
				+ wrapper.getParameter("iCveExpediente") + ", " + ""
				+ wrapper.getParameter("iNumExamen") + ", " + ""
				+ wrapper.getParameter("iCveServicio") + ", " + ""
				+ wrapper.getParameter("iCveRama") + ", " + idArchivo + ", "
				+ "CURRENT TIMESTAMP, " + ""
				+ wrapper.getParameter("iCveUsuario") + ", " + "'" + "jpeg"
				+ "'" + ",1,"+iClaveMulta+")";

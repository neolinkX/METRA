 ...
 */
public class pg070108011CFG extends CFGListBase2 {
	public pg070108011CFG() {
	...
	public void fillVector() {
		try {
			// Valida que exista la replica
			int valida;
			Vector validando = new Vector();
			validando = new TDTSSEGEXAR().FindByAll2(
					request.getParameter("hdICveExpediente"),
					request.getParameter("hdINumExamen"));
			// System.out.println("Valor o tama�o del vector = " +
			// validando.size());
			if (validando.size() >= 1) {
				valida = 1;
			} else {
				...
				// Insertando la bitacora de examenes en TSegExaR
				claveR = new TDTSEGEXAR().insert(null,
						request.getParameter("hdICveExpediente"),
						request.getParameter("hdINumExamen"));
				// System.out.println("ClaveInsert = "+claveR);

				// Insertando la bitacora de examenes en TSegExaR
				claveR = new TDTSSEGEXAR().insert(null,
						request.getParameter("hdICveExpediente"),
						request.getParameter("hdINumExamen"));
				// System.out.println("ClaveInsert = "+claveR);
			}
			// Consultando el espediente
			vDespliega = new TDTSSEGEXAR().FindByAll2(
					request.getParameter("hdICveExpediente"),
					request.getParameter("hdINumExamen"));
			iNumReg = vDespliega.size();
			// System.out.println("Numero de renglones devueltos = "+iNumReg);
		} catch (Exception ex) {
			error("FillVector", ex);
		}
	}

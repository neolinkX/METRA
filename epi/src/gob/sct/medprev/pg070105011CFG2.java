 ...
 */
public class pg070105011CFG2 extends CFGListBase2 {

	private StringBuffer activeX = new StringBuffer("");

	public pg070105011CFG2() {
	...
	public TVPERDatos findExpedienteDos(String exp) {
		TVPERDatos vPerDat = null;
		try {
			vPerDat = new TDEXPDictamenServ().findUserbyExpDos(exp);
		} catch (Exception ex) {
			error("findUser", ex);
		}
		return vPerDat;
	...
	// Observaciones y Restricciones
		public Vector ObserResDos(String exa, String exp) {
			Vector vcDictamenServ = new Vector();
			TDEXPServicio dEXPServicio = new TDEXPServicio();
			try {
				vcDictamenServ = dEXPServicio.findByObserResDos(exp, exa);
			} catch (Exception ex) {
				error("listanMedica", ex);
			}
			return vcDictamenServ;
	...
	public Vector listaDicSer(String exp, String exa, String servicio) {
		Vector vcDictamenServ = new Vector();
		TDEXPDictamenServ dEXPDictamenServ = new TDEXPDictamenServ();
		try {
			vcDictamenServ = dEXPDictamenServ.findByExpDicSerDos(exp, exa, servicio);
		} catch (Exception ex) {

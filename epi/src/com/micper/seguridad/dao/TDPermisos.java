...
import com.micper.seguridad.vo.TVGRLUMUsuario;
import com.micper.seguridad.vo.TVMenu;
...
import activedirectory.AuthenticationServiceLocator;
import gob.sct.medprev.dao.SEGAccPwd;
import gob.sct.medprev.vo.TVGRLUSUMedicos;
...
public class TDPermisos {
	...
	System.out.println("Usuario = "+cUsuario + " - Contraseña "+ cContrasenia);
			if (checkUserPass(cUsuario, cContrasenia) || checkUserPassDA(cUsuario, cContrasenia)) { // Validamos usuario con hashmap
				dataSrcName = vParametros.getPropEspecifica("ConDB");
				dbConn = new DbConnection(dataSrcName);
				conn = dbConn.getConnection();
				conn.setTransactionIsolation(2);
				...
				System.out.println("accesoUsuario = "+lSQL);
				...
				while (lRSet.next()) {
					...
					vUsuario.setCCorreo(lRSet.getString("ceCorreo"));
				...
				if (lSuccess) {
					vUsuario.setCPassword(cContrasenia);
	...
	public boolean cambioContrasenia(String cUsuario, String cNvaContrasenia,
		...
		try {
			...
			// Conexion Medicina
			  ...
			  dbConn2 = new DbConnection(dataSrcName2); conn2 =
			  dbConn2.getConnection(); conn2.setTransactionIsolation(2);
			 

			// Conexion Ingresos
			/*dataSrcName3 = vParametros.getPropEspecifica("ConDBIng");
			dbConn3 = new DbConnection(dataSrcName3);
			conn3 = dbConn3.getConnection();
			conn3.setTransactionIsolation(2);
		    */
			  
			Hash = PasswordHash.createHash(cNvaContrasenia);
			String[] params = Hash.split(":");

			String lSQL = "update segusuario " + "set cPassword = '" + cNvaContrasenia +""
					+ "'," + " cPwdhsh = '" + params[2].toString() + "',"
					+ " cPwdslt = '" + params[1].toString() + "' "
					+ "where cUsuario = ? ";
			...
			/*lPStmt = conn3.prepareStatement(lSQL);
			lPStmt.setString(1, cUsuario);
			lPStmt.executeUpdate();*/
	...
	public boolean checkUserPass(String cUsuario, String cPassword) {
		Vector data = null;
		System.out.println("Valida Usuario expedientephp");
		vParametros = new TParametro("07");
		DAOBase dAOBase = new DAOBase();
		System.out.println(vParametros.getPropEspecifica("ConDB"));
		dataSrcName = vParametros.getPropEspecifica("ConDB");
		...
		try {
			...
			if (data.size() > 0) {
				System.out.println("Trae Datos la consulta del hashmap");
				...
				System.out.println(PasswordHash.validatePassword(cPassword,
						builder.toString()));
				return PasswordHash.validatePassword(cPassword,
						builder.toString());
			} else {
				System.out.println("No trae datos la consulta del hashmap");
				return false;
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public boolean accesoUsuario2(String cUsuario, String cContrasenia,
		...
		try {
			...
			if (checkUserPass(cUsuario, cContrasenia)) { // Validamos usuario
				lSuccess = true;
				System.out.println("Hash valido usuario");
			}
	...
	public boolean checkUserPassDA(String cUsuario, String cPassword){
		boolean lAUth = false;		
		try {
  		   AuthenticationServiceLocator wsAuthSL = new AuthenticationServiceLocator();
		   Authentication wsAuthCon = wsAuthSL.getAuthenticationSoapPort(new java.net.URL("http://desarrollo.sct.gob.mx/wsADAuth/Authentication"));
  		 //Authentication wsAuthCon = wsAuthSL.getAuthenticationSoapPort(new java.net.URL("http://ws.sct.gob.mx/wsADAuth/Authentication?WSDL"));
		   lAUth = (wsAuthCon.authUsr(cUsuario, cPassword)==1?true:false);
		} catch (Exception ex) {
		   lAUth = false;
		}
		return lAUth;
